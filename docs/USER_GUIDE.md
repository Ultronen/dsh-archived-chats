...
Open an existing workspace row's ellipsis menu and choose **Archive chats**. The plugin automatically prepares that workspace and opens one confirmation naming the workspace, eligible-chat count, and the **Archived** destination in Session Archive. When applicable, it also shows how many chats currently in use will be skipped without listing chat identities. There is no workspace selector, session preview list, or second confirmation.
