...
1. 浏览器选择目标和已归档 ID，通过受保护的 interop/export 表单请求下载。
2. Host 重新确认每个 ID 仍是可见归档会话，逐条读取权威持久层记录并投影为 Codex 或 Claude Code JSONL。
3. 浏览器以 Blob 下载响应。输出明确定位为可读迁移或交接稿；目前不承诺目标工具原生 resume，也不包含附件二进制。
