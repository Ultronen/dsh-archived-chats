...
// packages emit.
//
// What this plugin is: an archived-chat settings section, a workspace-row
// archive action, and a frame-wide archive-success notice. The stock DSH
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function resolveWorkspaceArchiveRuntime(requireFn) {
			let primitives = null;
			let store = null;
			try { primitives = requireFn("@deepseek-ai/dsh-client-ui-primitives"); } catch { primitives = null; }
			try { store = requireFn("@deepseek-ai/dsh-client-store"); } catch { store = null; }
			return Object.freeze({
				MenuAction: typeof primitives?.MenuAction === "function" ? primitives.MenuAction : null,
				defineStore: typeof store?.defineStore === "function" ? store.defineStore : null,
			});
		}
		const workspaceArchiveRuntime = resolveWorkspaceArchiveRuntime(require);
		...
		const zh = {
			...
			"workspaceArchive.action": "归档项目会话",
			...
			"workspaceArchive.confirmBody": "「{workspace}」中的会话将移至会话档案。项目本身不会改变。",
			...
			"workspaceArchive.empty": "这个项目中没有可归档的会话。",
		...
		const en = {
			...
			"workspaceArchive.confirmBody": "Chats in \"{workspace}\" will move to Session Archive. The workspace itself will not change.",
			...
			"workspaceArchive.empty": "This workspace has no chats available to archive.",
...
.dac-workspace-dialog{box-sizing:border-box;width:min(430px,calc(100vw - 32px));max-height:calc(100vh - 32px);overflow:auto;border-radius:16px;padding:18px;background:var(--dsw-alias-bg-layer-1)}
.dac-workspace-body{display:flex;flex-direction:column;gap:12px;padding-top:14px;color:var(--dsw-alias-label-primary)}
.dac-workspace-copy{margin:0;color:var(--dsw-alias-label-secondary);font-size:13px;line-height:20px;overflow-wrap:anywhere}
.dac-workspace-actions{display:flex;justify-content:flex-end;gap:8px;padding-top:4px}
.dac-workspace-actions .dac-btn,.dac-workspace-actions .dac-btn-danger{min-width:86px;border-radius:10px}
...
@media (max-width:480px){.dac-workspace-dialog{width:calc(100vw - 20px);padding:14px}.dac-workspace-actions{flex-direction:column-reverse}.dac-workspace-actions .dac-btn,.dac-workspace-actions .dac-btn-danger{width:100%}}
`;
		...
		function WorkspaceArchiveDialog({ t, workspaceId, workspaceTitle, restoreFocus, onClose, onApplied }) {
			...
			const targetRef = _react.useRef(workspaceId);
			targetRef.current = workspaceId;
			...
			const prepare = _react.useCallback(async () => {
				...
				setPreview(null); setResult(null); setError(null); setStage("preparing");
				try {
					const next = await previewWorkspaceArchive(target);
					if (!mountedRef.current || preparationRequestRef.current !== request || targetRef.current !== target) return;
					setPreview(next);
					setStage(Array.isArray(next?.sessions) && next.sessions.length > 0 ? "confirm" : "empty");
				} catch (reason) {
					if (mountedRef.current && preparationRequestRef.current === request && targetRef.current === target) { setError(reason); setStage("error"); }
				}
			}, [workspaceId]);
			_react.useEffect(() => {
			...
			const apply = async () => {
				...
				if (busyRef.current || stage !== "confirm" || sessions.length === 0 || typeof preview?.token !== "string" || typeof preview?.nonce !== "string") return;
				...
				try {
					...
					if (mountedRef.current && applyRequestRef.current === request) { setPreview(null); setError(reason); setStage("error"); }
				} finally {
			...
			const targetTitle = safeLabel(preview?.workspace) || workspaceTitle || workspaceId;
			const confirmTitle = t("workspaceArchive.confirmTitle").replace("{count}", String(count));
			const confirmBody = t("workspaceArchive.confirmBody").replace("{workspace}", targetTitle);
			...
			return (0, jsx.jsx)("div", { className: "dac-confirm-overlay", onClick: close, children: (0, jsx.jsxs)("div", {
				...
				children: [
					...
					stage === "confirm" && (0, jsx.jsx)("div", { className: "dac-workspace-body", children: (0, jsx.jsx)("p", { className: "dac-workspace-copy", children: confirmBody }) }),
					stage === "empty" && (0, jsx.jsx)("div", { className: "dac-workspace-body", role: "status", children: (0, jsx.jsx)("p", { className: "dac-workspace-copy", children: t("workspaceArchive.empty") }) }),
		...
		function WorkspaceArchiveAction({ workspaceId, workspaceTitle, selectAction, actions, rememberFocus, isDisposed, t }) {
			const MenuAction = workspaceArchiveRuntime.MenuAction;
			if (MenuAction === null) return null;
			return (0, jsx.jsx)(MenuAction, {
				label: t("workspaceArchive.action"),
				onSelect: () => selectAction((restoreFocus) => {
					if (isDisposed()) return;
					rememberFocus(restoreFocus);
					actions.open(workspaceId, workspaceTitle);
				}),
			});
		}

		function ArchiveShellOverlay({ controller, t, useStore, actions, restoreWorkspaceArchiveFocus, refreshWorkspaceArchiveConsumers }) {
			const selection = useStore((state) => state.workspaceArchive);
			const onApplied = async () => {
				try { await refreshWorkspaceArchiveConsumers(); }
				finally { actions.refresh(); }
			};
			return (0, jsx.jsxs)("div", { className: "dac-shell-overlays", children: [
				(0, jsx.jsx)(ArchiveNoticeOverlay, { controller, t }),
				selection !== null && (0, jsx.jsx)(WorkspaceArchiveDialog, {
					t,
					workspaceId: selection.workspaceId,
					workspaceTitle: selection.workspaceTitle,
		...
		function ArchivedChatsSection({ t, refreshSidebar, useStore }) {
			const workspaceArchiveRefresh = typeof useStore === "function" ? useStore((state) => state.workspaceArchiveRevision) : 0;
		...
		function apply(ctx) {
			...
			const supportsWorkspaceArchiveAction = workspaceArchiveRuntime.defineStore !== null && workspaceArchiveRuntime.MenuAction !== null;
			const workspaceArchiveStore = supportsWorkspaceArchiveAction ? workspaceArchiveRuntime.defineStore({
				init: () => ({ workspaceArchive: null, workspaceArchiveRevision: 0 }),
				actions: {
					open: (draft, workspaceId, workspaceTitle) => { draft.workspaceArchive = { workspaceId, workspaceTitle }; },
					close: (draft) => { draft.workspaceArchive = null; },
					refresh: (draft) => { draft.workspaceArchiveRevision += 1; },
				},
			}) : null;
			let disposed = false;
			let workspaceArchiveRestoreFocus = null;
			if (workspaceArchiveStore !== null) ctx.effect(() => () => {
				disposed = true;
				workspaceArchiveRestoreFocus = null;
			}, "archived-chats: workspace archive action cleanup");
			const rememberWorkspaceArchiveFocus = (callback) => { workspaceArchiveRestoreFocus = typeof callback === "function" ? callback : null; };
			const restoreWorkspaceArchiveFocus = () => {
				const callback = workspaceArchiveRestoreFocus;
				workspaceArchiveRestoreFocus = null;
				callback?.();
			};
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				label: () => t("nav"),
				locale: SETTINGS_NS,
				...(workspaceArchiveStore !== null ? { store: workspaceArchiveStore } : {}),
			...
			ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				...
				locale: SETTINGS_NS,
				...(workspaceArchiveStore !== null ? { store: workspaceArchiveStore } : {}),
				inject: () => workspaceArchiveStore === null ? { controller: archiveNotice } : {
					controller: archiveNotice,
					restoreWorkspaceArchiveFocus,
					refreshWorkspaceArchiveConsumers,
				}
			}, workspaceArchiveStore === null ? ArchiveNoticeOverlay : ArchiveShellOverlay));
			if (workspaceArchiveStore !== null) ctx.slots.inject("sidebar.workspaces.workspace.action", () => ctx.slots.register({
				name: "sidebar.workspaces.workspace.action",
				id: "archived-chats-workspace-archive",
				order: 30,
				label: () => t("workspaceArchive.action"),
				locale: SETTINGS_NS,
				store: workspaceArchiveStore,
				inject: () => ({
					rememberFocus: rememberWorkspaceArchiveFocus,
					isDisposed: () => disposed,
				}),
