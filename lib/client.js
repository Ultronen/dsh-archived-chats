...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"workspaceArchive.chooseBody": "选择要批量归档会话的工作区。",
			"workspaceArchive.eligibleCount": "{count} 个可归档会话",
			"workspaceArchive.liveCount": "{count} 个运行中会话",
			"workspaceArchive.noWorkspaces": "当前没有可选择的工作区。",
		...
		const en = {
			...
			"workspaceArchive.chooseBody": "Choose the workspace whose chats you want to archive.",
			"workspaceArchive.eligibleCount": "{count} chats available",
			"workspaceArchive.liveCount": "{count} running chats",
			"workspaceArchive.noWorkspaces": "There are no workspaces to choose from.",
...
.dac-workspace-choice{display:flex;align-items:center;justify-content:space-between;gap:16px;width:100%;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:inherit;text-align:left;padding:11px 13px;cursor:pointer}
.dac-workspace-choice:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.12))}
.dac-workspace-choice:focus-visible{outline:2px solid var(--dsw-alias-interactive-primary,#6e6ef7);outline-offset:2px}
		...
		function WorkspaceArchiveChooserDialog({ t, onChoose, onClose }) {
			...
			const load = _react.useCallback(async () => {
				...
				try {
					...
					setState({ status: "ready", workspaces: Array.isArray(result?.workspaces) ? result.workspaces : [] });
				} catch {
			...
			return (0, jsx.jsx)("div", { className: "dac-confirm-overlay", onClick: onClose, children: (0, jsx.jsxs)("div", {
				...
				children: [
					...
					(0, jsx.jsxs)("div", { className: "dac-workspace-body", children: [
						...
						state.status === "ready" && state.workspaces.length > 0 && (0, jsx.jsx)("ul", { className: "dac-workspace-choices", children: state.workspaces.map((workspace) => (0, jsx.jsx)("li", { children: (0, jsx.jsxs)("button", {
							type: "button", className: "dac-workspace-choice", "data-workspace-archive-choice": String(workspace.id),
							onClick: () => onChoose?.({ id: String(workspace.id), title: typeof workspace.title === "string" ? workspace.title : null }),
							children: [
								(0, jsx.jsx)("span", { className: "dac-workspace-choice-title", children: typeof workspace.title === "string" && workspace.title !== "" ? workspace.title : String(workspace.id) }),
					...
					(0, jsx.jsx)("div", { className: "dac-workspace-actions", children: (0, jsx.jsx)("button", { ref: cancelRef, type: "button", className: "dac-btn", onClick: onClose, children: t("confirm.cancel") }) })
				]
			}) });
		}
		...
		function ArchivedChatsSection({ t, refreshSidebar, refreshWorkspaceArchiveConsumers }) {
			...
			const deleteReturnFocusRef = _react.useRef(null);
				...
				const metaReturnFocusRef = _react.useRef(null);
					...
					const chooseWorkspaceArchive = (workspace) => {
						if (typeof workspace?.id !== "string" || workspace.id === "") return;
						setWorkspaceArchive({ stage: "confirm", workspaceId: workspace.id, workspaceTitle: workspace.title });
					};
			...
			const runPermanentDelete = async (ids, finishSelectionAfter = false) => {
				if (ids.length === 0) return;
				markBusy(ids, true);
				setNotice(null);
				try {
			...
			const askPermanentDelete = (ids, finishSelectionAfter = false) => {
			if (ids.length === 0) return;
			deleteReturnFocusRef.current = document.activeElement;
			setConfirm({
				...
				(0, jsx.jsxs)("div", {
						...
						children: [
							...
							(0, jsx.jsxs)("div", {
									...
									(0, jsx.jsx)("button", {
										...
										className: "dac-bulk-btn dac-danger",
										disabled: selectedBusy,
										onClick: () => askPermanentDelete(selectedIds, true),
					...
					pageMode === "lineage" && (0, jsx.jsx)(ArchiveRelationshipsPanel, { t }),
						...
						workspaceArchive?.stage === "confirm" && (0, jsx.jsx)(WorkspaceArchiveDialog, {
							...
							workspaceTitle: workspaceArchive.workspaceTitle,
					...
					confirm !== null && (0, jsx.jsx)(ConfirmDialog, {
						...
						returnFocus: deleteReturnFocusRef.current,
						fallbackFocusRef: pageHeadingRef,
						onConfirm: () => confirm.permanent ? runPermanentDelete(confirm.ids, confirm.finishSelectionAfter === true) : runDelete(confirm.ids, confirm.finishSelectionAfter === true),
