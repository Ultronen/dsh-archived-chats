	...
	factory: (require) => {
		...
		function filterLineageForest(roots, query) {
			...
			const visit = (node) => {
				...
				const own = normalized === "" || [node?.id, node?.title]
					.some((value) => String(value ?? "").toLocaleLowerCase("en-US").includes(normalized));
				return own || children.length > 0 ? { ...node, children } : null;
			};
			return (Array.isArray(roots) ? roots : []).map(visit).filter(Boolean);
		}
...
`;
		...
		function RetentionPreviewDialog({ preview, selected, t, busy, onToggle, onApply, onCancel }) {
			const dialogRef = _react.useRef(null);
			const cancelRef = _react.useRef(null);
			...
			return (0, jsx.jsx)("div", { className: "dac-confirm-overlay", onClick: onCancel, children: (0, jsx.jsxs)("div", {
				ref: dialogRef, className: "dac-confirm dac-retention-dialog", role: "dialog", "aria-modal": "true", "aria-labelledby": "dac-retention-title", tabIndex: -1, onClick: (event) => event.stopPropagation(), children: [
					...
					candidates.length > 0 && (0, jsx.jsx)("div", { className: "dac-retention-list", children: candidates.map((item) => (0, jsx.jsxs)("label", { className: `dac-retention-row${item.action === "purge-trash" ? " dac-danger" : ""}`, children: [
						...
						(0, jsx.jsxs)("span", { children: [(0, jsx.jsx)("strong", { children: t(item.action === "purge-trash" ? "retention.trashCandidate" : "retention.snapshotCandidate") }), (0, jsx.jsx)("small", { children: `${item.sessionId} · ${formatBytes(item.bytes)}` })] })
					] }, item.key)) }),
					(0, jsx.jsxs)("div", { className: "dac-confirm-actions", children: [
		...
		function StorageRetentionPanel({ t }) {
			...
			const openPreview = async () => {
				setBusy(true); setMessage(null);
				try { const value = await previewRetention(); setPreview(value); setSelected(defaultRetentionSelection(value.candidates)); }
				catch (error) { setMessage(String(error.message ?? error)); }
				finally { setBusy(false); }
			};
			const applySelected = async () => {
				setBusy(true);
				try { await applyRetention(preview.token, preview.nonce, [...selected]); setPreview(null); setSelected(new Set()); await load(); }
				catch (error) { setMessage(String(error.message ?? error)); }
				finally { setBusy(false); }
			};
			...
			const cards = [["insights.sessions", value.summary.sessionBytes], ["insights.snapshots", value.summary.snapshotBytes], ["insights.total", value.summary.totalMeasuredBytes], ["insights.duplicate", value.summary.duplicateSnapshotBytes]];
			return (0, jsx.jsxs)("div", { className: "dac-mode-panel", children: [
				(0, jsx.jsx)("div", { className: "dac-insights-grid", children: cards.map(([label, bytes]) => (0, jsx.jsxs)("div", { className: "dac-insights-card", children: [(0, jsx.jsx)("span", { children: t(label) }), (0, jsx.jsx)("strong", { children: formatBytes(bytes) })] }, label)) }),
				...
				preview && (0, jsx.jsx)(RetentionPreviewDialog, { preview, selected, t, busy, onToggle: (key, checked) => setSelected((current) => setVisibleSelection(current, [key], checked)), onApply: applySelected, onCancel: () => setPreview(null) })
			] });
		}

		function LineageTreeNodes({ nodes, collapsed, onToggle }) {
			return (0, jsx.jsx)(jsx.Fragment, { children: nodes.map((node) => (0, jsx.jsxs)("div", { role: "treeitem", "aria-expanded": node.children.length > 0 ? !collapsed.has(node.id) : void 0, className: "dac-lineage-node", children: [
				(0, jsx.jsxs)("div", { className: "dac-lineage-row", children: [node.children.length > 0 && (0, jsx.jsx)("button", { type: "button", onClick: () => onToggle(node.id), children: collapsed.has(node.id) ? "+" : "−" }), (0, jsx.jsx)("strong", { children: node.title ?? node.id }), (0, jsx.jsx)("span", { className: `dac-trash-status dac-lineage-${node.status}`, children: node.status }), (0, jsx.jsx)("small", { children: node.id })] }),
				!collapsed.has(node.id) && node.children.length > 0 && (0, jsx.jsx)("div", { role: "group", className: "dac-lineage-children", children: (0, jsx.jsx)(LineageTreeNodes, { nodes: node.children, collapsed, onToggle }) })
			] }, node.id)) });
		}
		...
		function LineagePanel({ t }) {
			...
			return (0, jsx.jsxs)("div", { className: "dac-mode-panel", children: [
				...
				(state.value.diagnostics ?? []).length > 0 && (0, jsx.jsx)("div", { className: "dac-warn", role: "status", children: (state.value.diagnostics ?? []).map((item) => `${item.code}: ${item.sessionId}`).join(" · ") }),
				roots.length === 0 ? (0, jsx.jsx)("div", { className: "dac-empty", children: t("lineage.empty") }) : (0, jsx.jsx)("div", { role: "tree", className: "dac-lineage-tree", children: (0, jsx.jsx)(LineageTreeNodes, { nodes: roots, collapsed, onToggle: (id) => setCollapsed((current) => setVisibleSelection(current, [id], !current.has(id))) }) })
			] });
		}
