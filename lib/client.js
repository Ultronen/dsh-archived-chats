...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			"tag.input": "Tags (comma-separated)",
			"tag.placeholder": "Comma-separated, up to 8",
			"note.label": "Note",
		...
		function filterByTag(session, tag, locale) {
			const normalized = String(tag ?? "").toLocaleLowerCase(locale);
			if (normalized === "" || normalized === "all") return true;
			return (Array.isArray(session.tags) ? session.tags : [])
				.some((value) => String(value).toLocaleLowerCase(locale) === normalized);
		}
...
@media (max-width:640px){.dac-selection-toggle{margin-left:0}.dac-bulkbar{align-items:flex-start;flex-direction:column}.dac-bulk-actions{width:100%;flex-wrap:wrap}.dac-row{align-items:flex-start}.dac-row-actions{gap:4px}.dac-unarchive{padding:5px 10px}.dac-summary{gap:6px}.dac-row-meta{gap:4px}}
`;
		...
		function IconEdit({ size = 14 }) {
			return (0, jsx.jsxs)("svg", { ...svgProps(size), children: [(0, jsx.jsx)("path", { d: "M17 3a2.8 2.8 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" }), (0, jsx.jsx)("path", { d: "m15 5 4 4" })] });
		}
		//#endregion
		...
		function MetadataDialog({ session, t, busy, returnFocus, fallbackFocusRef, onSave, onCancel }) {
			...
			const [tagsText, setTagsText] = _react.useState(Array.isArray(session.tags) ? session.tags.join(", ") : "");
			...
			const overTags = parsedTags.length > 8 || parsedTags.some((tag) => Array.from(tag).length > 24);
			...
			return (0, jsx.jsx)("div", {
				...
				children: (0, jsx.jsxs)("div", {
					...
					children: [
						...
						(0, jsx.jsx)("div", { id: "dac-meta-limits", className: "dac-meta-limits", children: tagsLimitLabel(t, parsedTags.length) }),
						...
						(0, jsx.jsxs)("div", {
							...
							children: [
								...
								(0, jsx.jsx)("button", { type: "button", className: "dac-btn-primary", disabled: busy || overTags || overNote, onClick: () => onSave(parseTags(tagsText), note.trim()), children: t("meta.save") })
							]
						})
					]
				})
		...
		function GroupSection({ group, t, collapsed, onToggleCollapsed, menuOpen, onToggleMenu, onUnarchive, onDelete, busy, selected, onToggleSelected, stats, metadataStatus, onEditMetadata }) {
		...
		function ArchivedChatsSection({ t, refreshSidebar }) {
			...
			const [tagFilter, setTagFilter] = _react.useState("all");
			...
			const tagOptions = _react.useMemo(() => {
				...
				for (const session of sessions ?? []) {
					for (const tag of Array.isArray(session.tags) ? session.tags : []) seen.add(tag);
				}
			...
			const groups = _react.useMemo(() => {
				...
				const visible = (sessions ?? []).filter((s) => {
					...
					if (tagFilter !== "all" && !filterByTag(s, tagFilter, locale)) return false;
			...
			const filtering = query.trim() !== "" || projectFilter !== "all" || typeFilter !== "all" || tagFilter !== "all";
		...
		exports.__test = { formatBytes, matchesArchivedSession, filterByTag, sortArchivedSessions, setVisibleSelection, reconcileSelection };
		exports.apply = apply;
