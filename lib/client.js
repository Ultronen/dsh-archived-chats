...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			"trash.status.legacy": "Legacy recovery copy",
		...
		function trashStatusLabel(t, row) {
			...
			if (row?.sourceKind === "legacy-snapshot") return t("trash.status.legacy");
...
`;
		...
		function MetadataDialog({ session, t, busy, returnFocus, fallbackFocusRef, onSave, onCancel }) {
							...
							children: [
								(0, jsx.jsxs)("div", { className: "dac-preview-head", children: [
									(0, jsx.jsxs)("div", { className: "dac-preview-heading", children: [
										...
										Number.isFinite(snapshotMs) && (0, jsx.jsx)("time", { dateTime: preview.snapshot.createdAt, children: `${t("preview.snapshotAt")}: ${formatDate(t, snapshotMs)}` })
									] }),
			...
			const restorableIds = group.items
				.filter((row) => row.state !== "purge-pending" && row.restorable !== false)
			...
			return (0, jsx.jsxs)("section", { className: "dac-group dac-trash-group", children: [
				...
				collapsed !== true && (0, jsx.jsx)("div", { className: "dac-list", children: group.items.map((row) => {
					...
					const legacy = row.sourceKind === "legacy-snapshot";
					const restorable = !pending && row.restorable !== false;
					...
					return (0, jsx.jsxs)("article", { className: "dac-row dac-trash-row", children: [
						...
						(0, jsx.jsxs)("div", { className: "dac-row-actions dac-trash-actions", children: [
							(0, jsx.jsx)("button", { type: "button", className: "dac-iconbtn", "aria-label": previewOpenLabel(t, title), disabled: rowBusy || (legacy && !restorable), onClick: () => onPreview(row), children: (0, jsx.jsx)(IconEye, {}) }),
		...
		function ArchivedChatsSection({ t, refreshSidebar, refreshWorkspaceArchiveConsumers }) {
			...
			const openPreview = async (session, scope = "archive") => {
				if (scope === "trash" && session?.sourceKind === "legacy-snapshot") {
					if (session.restorable === false || typeof session.legacySnapshotId !== "string") return;
					await openHistoryPreview({
						sessionId: session.sourceSessionId ?? session.sessionId,
						title: session.title,
						workspace: session.workspace ?? null,
					}, {
						snapshotId: session.legacySnapshotId,
						createdAt: session.createdAt === null || session.createdAt === undefined
							? session.trashedAt : new Date(session.createdAt).toISOString(),
					});
					return;
				}
