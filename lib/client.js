...
.dac-head-actions{display:flex;flex:1 1 auto;min-width:0;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap}
...
`;
		...
		function MetadataDialog({ session, t, busy, returnFocus, fallbackFocusRef, onSave, onCancel }) {
			...
			function InteropExportDialog({ count, target, t, busy, onTarget, onConfirm, onCancel }) {
				const dialogRef = _react.useRef(null);
				const cancelRef = _react.useRef(null);
				_react.useEffect(() => {
					(cancelRef.current ?? dialogRef.current)?.focus?.();
					...
					children: (0, jsx.jsxs)("div", {
						...
						children: [
							...
							(0, jsx.jsxs)("div", { className: "dac-interop-report", children: [
								(0, jsx.jsx)("strong", { children: t("interop.report") }),
								(0, jsx.jsx)("span", { children: `${t("interop.sessions")}${isZh(t) ? "：" : ": "}${count}` }),
								(0, jsx.jsx)("span", { children: `${t("interop.fidelity")}${isZh(t) ? "：" : ": "}${t("interop.fidelity.readable")}` }),
							] }),
							(0, jsx.jsx)("div", { className: "dac-interop-limit", children: t("interop.export.nativeUnsupported") }),
							(0, jsx.jsxs)("div", { className: "dac-confirm-actions", children: [
								(0, jsx.jsx)("button", { ref: cancelRef, type: "button", className: "dac-btn", onClick: onCancel, children: t("import.cancel") }),
								(0, jsx.jsx)("button", { type: "button", className: "dac-btn-primary", disabled: busy, onClick: onConfirm, children: t("interop.export.download") }),
							] }),
						]
					})
				});
		...
		function ArchivedChatsSection({ t, refreshSidebar }) {
			...
			const [stats, setStats] = _react.useState({ status: "idle", summary: null, sessions: {} });
				...
				const [interop, setInterop] = _react.useState(() => ({ source: "codex", target: "codex", busy: false, preview: null, exportIds: null }));
			...
			const openMetadataEditor = (session) => {
				...
				const interopFile = async (file) => {
					if (!file) return;
					setInterop((current) => ({ ...current, busy: true }));
					setNotice(null);
					try {
						const inspected = await submitInteropFile(file, interop.source);
						const previewSessions = (Array.isArray(inspected.sessions) ? inspected.sessions : []).map((session) => {
							const warnings = [];
							if (Array.isArray(session.attachments) && session.attachments.length > 0) warnings.push("attachments-not-included");
							if (Array.isArray(session.losses) && session.losses.length > 0) warnings.push("interop-losses");
							return {
								id: session.id,
								title: session.title,
						...
						const report = {
							source: inspected.report?.source ?? interop.source,
							summary: {
				...
				const runInteropExport = async () => {
					if (!Array.isArray(interop.exportIds) || interop.exportIds.length === 0) return;
					setInterop((current) => ({ ...current, busy: true }));
					setNotice(null);
					try {
						await downloadInteropExport(interop.exportIds, interop.target);
						setInterop((current) => ({ ...current, exportIds: null }));
						...
						children: [
							(0, jsx.jsx)("h2", { ref: pageHeadingRef, tabIndex: -1, className: "dac-title", children: t("page.title") }),
								(0, jsx.jsxs)("div", {
									...
									(0, jsx.jsx)(Select, {
										value: interop.source,
										onChange: (source) => setInterop((current) => ({ ...current, source })),
										ariaLabel: t("interop.source.label"),
										options: [
											{ value: "codex", label: t("interop.source.codex") },
											{ value: "claude", label: t("interop.source.claude") },
										],
									...
									(0, jsx.jsxs)("button", {
										type: "button",
										className: "dac-import",
										disabled: interop.busy,
										onClick: () => interopInputRef.current?.click?.(),
										children: [(0, jsx.jsx)(IconUpload, {}), (0, jsx.jsx)("span", { children: t("interop.import.action") })]
									}),
									(0, jsx.jsxs)("button", {
										type: "button",
										className: "dac-exportall",
										disabled: allIds.length === 0 || interop.busy,
										onClick: () => setInterop((current) => ({ ...current, exportIds: selectedIds.length > 0 ? selectedIds : allIds })),
										children: [(0, jsx.jsx)(IconDownload, {}), (0, jsx.jsx)("span", { children: t("interop.export.action") })]
									}),
									...
									(0, jsx.jsx)("input", {
										...
										hidden: true,
										onChange: (event) => { const file = event.target.files?.[0]; void interopFile(file); event.target.value = ""; }
									}),
								]
							}),
				(0, jsx.jsxs)("div", {
					...
					groups.map((group) => (0, jsx.jsx)(GroupSection, {
						...
						interop.exportIds !== null && (0, jsx.jsx)(InteropExportDialog, {
							...
							t,
							busy: interop.busy,
							onTarget: (target) => setInterop((current) => ({ ...current, target })),
							onConfirm: runInteropExport,
							onCancel: () => setInterop((current) => ({ ...current, exportIds: null })),
						}),
		...
		exports.__test = { formatBytes, matchesArchivedSession, filterByTag, sortArchivedSessions, setVisibleSelection, reconcileSelection, markArchiveDialog, submitExport, submitImportFile, submitInteropFile, downloadInteropExport, editIconSpec: EDIT_ICON_SPEC };
		exports.apply = apply;
