...
`;
		...
		function WorkspaceArchiveDialog({ t, returnFocus, fallbackFocusRef, onClose, onApplied }) {
			...
			_react.useEffect(() => { void loadWorkspaces(); return () => { mountedRef.current = false; loadRequestRef.current += 1; previewRequestRef.current = { sequence: previewRequestRef.current.sequence + 1, workspaceId: "" }; }; }, [loadWorkspaces]);
