	...
	factory: (require) => {
		...
		const zh = {
			...
			"nav": "已归档的聊天",
				...
				"lineage.loading": "正在读取会话血缘…",
				"lineage.error": "会话血缘暂不可用",
				"lineage.search": "搜索标题或会话 ID",
				"lineage.empty": "没有可显示的会话关系",
				"lineage.status.active": "活动",
				"lineage.status.archived": "归档",
				"lineage.status.trash": "回收站",
				"lineage.status.missing": "缺失",
		...
		const en = {
			...
			"nav": "Archived Chats",
				...
				"insights.snapshot.active": "Active protection snapshot",
				"insights.snapshot.history": "Historical protection snapshot",
				"insights.snapshot.degraded": "Degraded protection snapshot",
				"insights.snapshot.original": "Original chat",
				"insights.snapshot.created": "Created",
				"insights.snapshot.id": "Snapshot ID",
				"retention.history": "Historical snapshots per session",
				...
				"lineage.loading": "Loading session lineage…",
				"lineage.error": "Session lineage is unavailable",
				"lineage.search": "Search titles or session IDs",
				"lineage.empty": "No session relationships to display",
				"lineage.status.active": "Active",
				"lineage.status.archived": "Archived",
				"lineage.status.trash": "Recycle Bin",
				"lineage.status.missing": "Missing",
		...
		function filterLineageForest(roots, query) {
			...
			while (stack.length > 0) {
				...
				const own = normalized === "" || [current.node?.id, current.node?.title].some((value) => String(value ?? "").toLocaleLowerCase("en-US").includes(normalized));
...
.dac-insights-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.dac-insights-card{display:flex;flex-direction:column;gap:5px;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-layer-1);padding:14px}.dac-insights-card span{color:var(--dsw-alias-label-tertiary);font-size:12px}.dac-insights-card strong{color:var(--dsw-alias-label-primary);font-size:18px}.dac-policy{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;padding:14px}.dac-policy label{display:flex;flex-direction:column;gap:5px;color:var(--dsw-alias-label-secondary);font-size:12px}.dac-policy input{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);padding:7px 9px}.dac-policy-actions{grid-column:1/-1;display:flex;justify-content:flex-end;gap:8px}.dac-retention-dialog{width:min(620px,calc(100vw - 48px));max-height:min(720px,calc(100vh - 48px))}.dac-retention-list{display:flex;flex-direction:column;gap:8px;overflow:auto}.dac-retention-row{display:flex;gap:10px;border:1px solid var(--dsw-alias-border-l2);border-radius:9px;padding:9px}.dac-retention-row span{display:flex;flex-direction:column}.dac-retention-row small{color:var(--dsw-alias-label-tertiary)}.dac-retention-row.dac-danger strong{color:var(--dsw-alias-state-error-primary)}.dac-lineage-tree{display:flex;flex-direction:column;gap:6px}.dac-lineage-node{min-width:0}.dac-lineage-row{display:flex;align-items:center;gap:8px;border:1px solid var(--dsw-alias-border-l2);border-radius:9px;background:var(--dsw-alias-bg-layer-1);padding:8px 10px}.dac-lineage-row button{border:0;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer}.dac-lineage-row strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dac-lineage-row small{margin-left:auto;color:var(--dsw-alias-label-tertiary);overflow:hidden;text-overflow:ellipsis}.dac-lineage-children{margin:6px 0 0 22px;display:flex;flex-direction:column;gap:6px}.dac-lineage-missing{color:var(--dsw-alias-state-error-primary)}
...
.dac-snapshot-row .dac-row-main{gap:3px}.dac-snapshot-id{white-space:normal;overflow-wrap:anywhere;word-break:break-word}.dac-lineage-row{flex-wrap:wrap}.dac-lineage-row>strong{flex:1 1 240px;white-space:normal;overflow:visible;text-overflow:clip;overflow-wrap:anywhere;word-break:break-word}.dac-lineage-detail{display:flex;align-items:flex-end;gap:10px;flex:1 1 100%;min-width:0}.dac-lineage-detail-copy{display:flex;flex:1 1 auto;min-width:0;flex-direction:column;gap:2px}.dac-lineage-id{white-space:normal;overflow-wrap:anywhere;word-break:break-word}.dac-lineage-copy{flex:none;border:1px solid var(--dsw-alias-border-l2);border-radius:7px;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:11px;line-height:16px;padding:3px 8px;cursor:pointer}.dac-lineage-copy:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.12));color:var(--dsw-alias-label-primary)}
...
`;
		...
		function StorageRetentionPanel({ t }) {
			...
			const value = state.value;
				const cards = [["insights.sessions", formatBytes(value.summary.sessionBytes)], ["insights.snapshots", formatBytes(value.summary.snapshotBytes)], ["insights.total", formatBytes(value.summary.totalMeasuredBytes)], ["insights.duplicate", formatBytes(value.summary.duplicateSnapshotBytes)], ["insights.unavailable", `${value.summary.sessionUnavailableCount} / ${value.summary.degradedSnapshotCount}`]];
			return (0, jsx.jsxs)("div", { className: "dac-mode-panel", children: [
				(0, jsx.jsx)("div", { className: "dac-insights-grid", children: cards.map(([label, display]) => (0, jsx.jsxs)("div", { className: "dac-insights-card", children: [(0, jsx.jsx)("span", { children: t(label) }), (0, jsx.jsx)("strong", { children: display })] }, label)) }),
				(0, jsx.jsx)("div", { className: "dac-list", children: (value.sessions ?? []).map((row) => (0, jsx.jsxs)("div", { className: "dac-row", children: [(0, jsx.jsxs)("div", { className: "dac-row-main", children: [(0, jsx.jsx)("strong", { children: row.title ?? row.id }), (0, jsx.jsx)("span", { className: "dac-row-date", children: `${row.workspaceTitle ?? "—"} · ${row.scope}` })] }), (0, jsx.jsx)("span", { className: "dac-row-size", children: row.status === "ready" ? formatBytes(row.sizeBytes) : "—" })] }, `session:${row.id}`)) }),
				(0, jsx.jsx)(SnapshotInsightRows, { snapshots: value.snapshots, sessions: value.sessions, t }),
		...
		function flattenLineageNodes(nodes, collapsed) {
			const output = [];
			const stack = (Array.isArray(nodes) ? nodes : []).map((node) => ({ node, level: 1 })).reverse();
			while (stack.length > 0) {
				const current = stack.pop();
				output.push(current);
				if (collapsed.has(current.node.id)) continue;
				const children = Array.isArray(current.node.children) ? current.node.children : [];
				for (let index = children.length - 1; index >= 0; index -= 1) stack.push({ node: children[index], level: current.level + 1 });
			}
			return output;
		}

		function LineageTreeNodes({ nodes, collapsed, onToggle, onCopy, copiedId, t }) {
			return (0, jsx.jsx)(jsx.Fragment, { children: flattenLineageNodes(nodes, collapsed).map(({ node, level }) => (0, jsx.jsx)("div", { role: "treeitem", "aria-level": level, "aria-expanded": node.children.length > 0 ? !collapsed.has(node.id) : void 0, className: "dac-lineage-node", style: { marginLeft: `${Math.min(level - 1, 8) * 22}px` }, children:
				(0, jsx.jsxs)("div", { className: "dac-lineage-row", children: [
					node.children.length > 0 && (0, jsx.jsx)("button", { type: "button", onClick: () => onToggle(node.id), children: collapsed.has(node.id) ? "+" : "−" }),
					(0, jsx.jsx)("strong", { title: node.id, style: { whiteSpace: "normal", overflowWrap: "anywhere" }, children: node.title ?? node.id }),
					...
					(0, jsx.jsxs)("div", { className: "dac-lineage-detail", children: [
						(0, jsx.jsxs)("div", { className: "dac-lineage-detail-copy", children: [
							(0, jsx.jsx)("small", { className: "dac-lineage-id", children: node.id }),
							(0, jsx.jsx)("small", { children: `${t(`lineage.origin.${node.origin ?? "session"}`)} · d${node.delegationDepth ?? 0} · ${node.children.length} · ${formatDate(t, node.createdAt)}` })
						] }),
						(0, jsx.jsx)("button", { type: "button", className: "dac-lineage-copy", "aria-label": `${t("lineage.copyId")} ${node.id}`, onClick: () => onCopy?.(node.id), children: copiedId === node.id ? t("lineage.copiedId") : t("lineage.copyId") })
					] })
				] })
			}, node.id)) });
		}

		function LineagePanel({ t }) {
			...
			const load = _react.useCallback(async (signal) => {
				...
				try { const value = await fetchLineage(signal); if (!signal?.aborted) setState({ status: "ready", value }); }
			...
			const roots = filterLineageForest(state.value.roots, query);
			return (0, jsx.jsxs)("div", { className: "dac-mode-panel", children: [
				(0, jsx.jsx)("div", { className: "dac-search", children: (0, jsx.jsx)("input", { value: query, placeholder: t("lineage.search"), onChange: (event) => setQuery(event.target.value) }) }),
				(state.value.diagnostics ?? []).length > 0 && (0, jsx.jsx)("div", { className: "dac-row-meta", role: "status", children: (state.value.diagnostics ?? []).map((item) => (0, jsx.jsx)("span", { className: "dac-chip dac-summary-warn", children: `${t(`lineage.diagnostic.${item.code}`)}: ${item.sessionId}` }, `${item.code}:${item.sessionId}`)) }),
				roots.length === 0 ? (0, jsx.jsx)("div", { className: "dac-empty", children: t("lineage.empty") }) : (0, jsx.jsx)("div", { role: "tree", className: "dac-lineage-tree", children: (0, jsx.jsx)(LineageTreeNodes, { nodes: roots, collapsed, copiedId, t, onCopy: copyId, onToggle: (id) => setCollapsed((current) => setVisibleSelection(current, [id], !current.has(id))) }) })
			] });
		}
		...
		function ArchivedChatsSection({ t, refreshSidebar }) {
			...
			return (0, jsx.jsxs)("div", {
				...
				(0, jsx.jsxs)("div", {
					...
					pageMode === "lineage" && (0, jsx.jsx)(LineagePanel, { t }),
