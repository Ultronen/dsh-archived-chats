...
.dac-workspace-dialog{width:min(560px,calc(100vw - 32px));max-height:calc(100vh - 32px);overflow:auto}
...
`;
		...
		async function applyWorkspaceArchive(token, nonce) {
			return post("/workspace-archive/apply", { token, nonce });
		}
		...
		function WorkspaceArchiveDialog({ t, returnFocus, fallbackFocusRef, onClose, onApplied }) {
			...
			_react.useEffect(() => {
				let cancelled = false;
				void fetchWorkspaceArchiveWorkspaces().then((body) => {
					if (cancelled) return;
					setWorkspaces(Array.isArray(body?.workspaces) ? body.workspaces : []);
					setStage("select");
				}).catch((reason) => { if (!cancelled) { setError(reason); setStage("error"); } });
				return () => { cancelled = true; };
			}, []);
			const requestPreview = async () => {
				if (workspaceId === "" || busy) return;
				setBusy(true); setError(null);
				try { setPreview(await previewWorkspaceArchive(workspaceId)); setStage("preview"); }
			...
			const apply = async () => {
				...
				try {
					...
					try { await onApplied?.(applied); } catch { /* the completed result remains readable */ }
				} catch (reason) { setError(reason); setStage("error"); }
				finally { setBusy(false); }
			};
			...
			const resultRows = (label, rows) => Array.isArray(rows) && rows.length > 0 && (0, jsx.jsxs)("section", { className: "dac-workspace-result-group", children: [
				(0, jsx.jsx)("strong", { children: label }),
				(0, jsx.jsx)("ul", { children: rows.map((item) => (0, jsx.jsx)("li", { children: typeof item === "string" ? item : `${safeLabel(item)}${item?.reason ? ` · ${item.reason}` : item?.status ? ` · ${item.status}` : ""}` }, typeof item === "string" ? item : `${item?.id}:${item?.reason ?? item?.status ?? ""}`)) })
			] });
			return (0, jsx.jsx)("div", { className: "dac-confirm-overlay", onClick: close, children: (0, jsx.jsxs)("div", {
				...
				children: [
					...
					stage === "select" && (0, jsx.jsxs)("div", { className: "dac-workspace-body", children: [
						(0, jsx.jsx)("label", { children: t("workspaceArchive.choose") }),
						(0, jsx.jsxs)("select", { "data-workspace-archive-select": "1", className: "dac-select dac-select-fill", value: workspaceId, onChange: (event) => setWorkspaceId(event.target.value), children: [
							(0, jsx.jsx)("option", { value: "", children: t("workspaceArchive.choosePlaceholder") }),
							...workspaces.map((workspace) => (0, jsx.jsx)("option", { value: workspace.id, children: `${safeLabel(workspace)} · ${workspace.eligibleCount ?? 0}` }, workspace.id))
						] }),
					...
					stage === "preview" && (0, jsx.jsxs)("div", { className: "dac-workspace-body", children: [
						...
						previewSkipped.length > 0 && (0, jsx.jsxs)("div", { children: [(0, jsx.jsx)("span", { children: t("workspaceArchive.skipped") }), (0, jsx.jsx)("ul", { children: previewSkipped.map((item) => (0, jsx.jsx)("li", { children: `${safeLabel(item)} · ${item.reason === "session-live" ? t("workspaceArchive.sessionLive") : item.reason}` }, item.id)) })] }),
					...
					stage === "error" && (0, jsx.jsxs)("div", { className: "dac-workspace-body dac-error", role: "status", children: [
						...
						(0, jsx.jsx)("button", { type: "button", className: "dac-btn", onClick: () => { setError(null); setStage("select"); }, children: t("state.retry") })
					] })
				]
			}) });
		}
		...
		function ArchivedChatsSection({ t, refreshSidebar }) {
			...
			const workspaceArchiveApplied = _react.useCallback(async () => {
				await refresh();
				refreshSidebar?.();
				try { await refreshWorkspaceRegistry(); } catch { /* Host workspace view refresh is optional */ }
