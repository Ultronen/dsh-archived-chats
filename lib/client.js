...
.dac-preview-dialog{width:min(920px,calc(100vw - 48px));height:min(720px,calc(100vh - 48px));padding:0;gap:0;overflow:hidden}
...
.dac-preview-feed{min-width:0;overflow:auto;display:flex;flex-direction:column;gap:12px;padding:16px;background:var(--dsw-alias-bg-layer-2)}
.dac-preview-message{display:flex;flex-direction:column;gap:8px;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-layer-1);padding:12px}
.dac-preview-message-head{display:flex;align-items:center;justify-content:space-between;gap:8px;color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:16px}
.dac-preview-message-head strong{color:var(--dsw-alias-label-secondary);font-size:12px}
.dac-preview-segment{margin:0;color:var(--dsw-alias-label-primary);font:inherit;font-size:13px;line-height:21px;white-space:pre-wrap;overflow-wrap:anywhere}
.dac-preview-detail{border:1px solid var(--dsw-alias-border-l2);border-radius:9px;background:var(--dsw-alias-bg-layer-2);padding:7px 9px}
.dac-preview-detail summary{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;cursor:pointer}
.dac-preview-detail pre{margin:8px 0 0;color:var(--dsw-alias-label-primary);font:12px/18px ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;overflow-wrap:anywhere}
.dac-preview-detail.dac-error{border-color:var(--dsw-alias-state-error-primary)}
...
`;
		...
		async function fetchArchivePreview(sessionId, offset = 0, limit = 50) {
			return post("/preview", { sessionId, offset, limit });
		}
		...
		function MetadataDialog({ session, t, busy, returnFocus, fallbackFocusRef, onSave, onCancel }) {
			...
			function importWarningText(t, warning) {
				...
				function PreviewDialog({ preview, t, busy, returnFocus, fallbackFocusRef, onLoadMore, onCancel }) {
					...
					const segmentNode = (segment, key) => {
						if (segment.kind === "text") return (0, jsx.jsx)("p", { className: "dac-preview-segment", children: segment.text }, key);
						const label = segment.kind === "reasoning"
							? t("preview.reasoning")
							: segment.kind === "tool-call"
								? `${t("preview.toolCall")}: ${segment.label ?? ""}`
								: segment.kind === "tool-result"
									? `${t("preview.toolResult")}${segment.label ? `: ${segment.label}` : ""}`
									: segment.label ?? segment.kind;
						return (0, jsx.jsxs)("details", {
							className: `dac-preview-detail${segment.isError ? " dac-error" : ""}`,
							children: [
								(0, jsx.jsx)("summary", { children: label }),
								(0, jsx.jsx)("pre", { children: segment.text })
							]
						}, key);
					};
							...
							children: [
								(0, jsx.jsxs)("div", { className: "dac-preview-head", children: [
									(0, jsx.jsxs)("div", { className: "dac-preview-heading", children: [
										...
										(0, jsx.jsx)("span", { children: t("preview.title") })
									] }),
								...
								preview?.status === "ready" && messages.length > 0 && (0, jsx.jsxs)("div", { className: "dac-preview-layout", children: [
									(0, jsx.jsx)("nav", { className: "dac-preview-rail", "aria-label": t("preview.timeline"), children: messages.map((message, index) => (0, jsx.jsx)("button", {
										type: "button",
										"aria-label": previewJumpLabel(t, index),
										onClick: () => {
											try { document.getElementById(`dac-preview-message-${message.seq}`)?.scrollIntoView?.({ block: "start", behavior: "smooth" }); } catch { /* host DOM changed */ }
										},
										children: `#${index + 1}`
									}, `${message.seq}-${index}`)) }),
									(0, jsx.jsxs)("div", { className: "dac-preview-feed", children: [
										...messages.map((message, index) => (0, jsx.jsxs)("article", {
											id: `dac-preview-message-${message.seq}`,
											className: `dac-preview-message dac-preview-${message.role}`,
											children: [
												(0, jsx.jsxs)("div", { className: "dac-preview-message-head", children: [
													(0, jsx.jsx)("strong", { children: previewRoleLabel(t, message.role) }),
													(0, jsx.jsx)("span", { children: message.time === null ? `#${message.seq}` : formatDate(t, message.time) })
												] }),
												...(Array.isArray(message.segments) ? message.segments : []).map((segment, segmentIndex) => segmentNode(segment, `${message.seq}-${segmentIndex}`))
											]
										}, `${message.seq}-${index}`)),
		...
		function ArchivedChatsSection({ t, refreshSidebar }) {
			...
			const openPreview = async (session) => {
				...
				setPreview({ status: "loading", session, messages: [], total: 0, nextOffset: null });
				setPreviewBusy(true);
				try {
					const result = await fetchArchivePreview(session.id, 0, 50);
					setPreview({ ...result, status: "ready", session: result?.session ?? session, messages: Array.isArray(result?.messages) ? result.messages : [] });
				} catch (error) {
					setPreview({ status: "error", session, messages: [], total: 0, nextOffset: null, error: String(error.message ?? error) });
				} finally {
			...
			const loadMorePreview = async () => {
				...
				setPreviewBusy(true);
				try {
					const result = await fetchArchivePreview(preview.session.id, preview.nextOffset, 50);
			...
			const closePreview = _react.useCallback(() => setPreview(null), []);
		...
		exports.SETTINGS_NS = SETTINGS_NS;
			exports.__test = { formatBytes, matchesArchivedSession, filterByTag, sortArchivedSessions, setVisibleSelection, reconcileSelection, markArchiveDialog, submitExport, submitImportFile, fetchArchivePreview, fetchArchiveSearch, editIconSpec: EDIT_ICON_SPEC };
		exports.apply = apply;
