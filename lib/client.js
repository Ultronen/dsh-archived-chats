...
`;
		...
		async function fetchArchivePreview(sessionId, offset = 0, limit = 50, signal) {
			return post("/preview", { sessionId, offset, limit }, signal);
		}
		...
		function ArchivedChatsSection({ t, refreshSidebar }) {
			...
			const openPreview = async (session) => {
				...
				setPreviewBusy(true);
				try {
					const result = await fetchArchivePreview(session.id, 0, 50, request.controller.signal);
			...
			const loadMorePreview = async () => {
				...
				setPreviewBusy(true);
				try {
					const result = await fetchArchivePreview(sessionId, nextOffset, 50, request.controller.signal);
					if (!ownsPreviewRequest(request)) return;
