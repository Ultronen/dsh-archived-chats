...
`;
		...
		function RetentionPreviewDialog({ preview, selected, t, busy, returnFocus, onToggle, onApply, onCancel }) {
			...
			return (0, jsx.jsx)("div", { className: "dac-confirm-overlay", onClick: onCancel, children: (0, jsx.jsxs)("div", {
				ref: dialogRef, className: "dac-confirm dac-retention-dialog", role: "dialog", "aria-modal": "true", "aria-labelledby": "dac-retention-title", tabIndex: -1, onClick: (event) => event.stopPropagation(), children: [
					...
					candidates.length > 0 && (0, jsx.jsx)("div", { className: "dac-retention-list", children: candidates.map((item) => (0, jsx.jsxs)("label", { className: `dac-retention-row${item.action === "purge-trash" ? " dac-danger" : ""}`, children: [
		...
		function StorageRetentionPanel({ t }) {
			...
			const applySelected = async () => {
				setBusy(true);
				try { const result = await applyRetention(preview.token, preview.nonce, [...selected]); const failed = result?.failed ?? []; setMessage(failed.length > 0 ? failed.map((item) => `${item.key}: ${item.reason}`).join(" · ") : t("retention.applied")); setPreview(null); setSelected(new Set()); await load(); }
				catch (error) { setMessage(String(error.message ?? error)); }
				finally { setBusy(false); }
			};
			...
			return (0, jsx.jsxs)("div", { className: "dac-mode-panel", children: [
				...
				(0, jsx.jsx)("div", { className: "dac-list", children: (value.snapshots ?? []).map((row) => (0, jsx.jsxs)("div", { className: "dac-row", children: [(0, jsx.jsxs)("div", { className: "dac-row-main", children: [(0, jsx.jsx)("strong", { children: row.snapshotId }), (0, jsx.jsx)("span", { className: "dac-row-date", children: row.status === "ready" ? `${row.sessionId}${row.active ? " · active" : ""}` : row.code })] }), (0, jsx.jsx)("span", { className: "dac-row-size", children: row.status === "ready" ? formatBytes(row.totalBytes) : "—" })] }, `snapshot:${row.snapshotId}`)) }),
				...
				preview && (0, jsx.jsx)(RetentionPreviewDialog, { preview, selected, t, busy, returnFocus: previewReturnFocusRef.current, onToggle: (key, checked) => setSelected((current) => setVisibleSelection(current, [key], checked)), onApply: applySelected, onCancel: () => setPreview(null) })
			] });
		}
		...
		function LineageTreeNodes({ nodes, collapsed, onToggle, t }) {
			return (0, jsx.jsx)(jsx.Fragment, { children: flattenLineageNodes(nodes, collapsed).map(({ node, level }) => (0, jsx.jsx)("div", { role: "treeitem", "aria-level": level, "aria-expanded": node.children.length > 0 ? !collapsed.has(node.id) : void 0, className: "dac-lineage-node", style: { marginLeft: `${(level - 1) * 22}px` }, children:
				(0, jsx.jsxs)("div", { className: "dac-lineage-row", children: [node.children.length > 0 && (0, jsx.jsx)("button", { type: "button", onClick: () => onToggle(node.id), children: collapsed.has(node.id) ? "+" : "−" }), (0, jsx.jsx)("strong", { children: node.title ?? node.id }), (0, jsx.jsx)("span", { className: `dac-trash-status dac-lineage-${node.status}`, children: t(`lineage.status.${node.status}`) }), (0, jsx.jsx)("small", { children: `${node.id} · ${node.origin ?? "session"} · d${node.delegationDepth ?? 0} · ${node.children.length} · ${formatDate(t, node.createdAt)}` })] })
			}, node.id)) });
		}
		...
		function LineagePanel({ t }) {
			...
			return (0, jsx.jsxs)("div", { className: "dac-mode-panel", children: [
				...
				(state.value.diagnostics ?? []).length > 0 && (0, jsx.jsx)("div", { className: "dac-warn", role: "status", children: (state.value.diagnostics ?? []).map((item) => `${t(`lineage.diagnostic.${item.code}`)}: ${item.sessionId}`).join(" · ") }),
				roots.length === 0 ? (0, jsx.jsx)("div", { className: "dac-empty", children: t("lineage.empty") }) : (0, jsx.jsx)("div", { role: "tree", className: "dac-lineage-tree", children: (0, jsx.jsx)(LineageTreeNodes, { nodes: roots, collapsed, t, onToggle: (id) => setCollapsed((current) => setVisibleSelection(current, [id], !current.has(id))) }) })
			] });
		}
