...
`;
		...
		function LineageTreeNodes({ nodes, collapsed, onToggle, t }) {
			return (0, jsx.jsx)(jsx.Fragment, { children: flattenLineageNodes(nodes, collapsed).map(({ node, level }) => (0, jsx.jsx)("div", { role: "treeitem", "aria-level": level, "aria-expanded": node.children.length > 0 ? !collapsed.has(node.id) : void 0, className: "dac-lineage-node", style: { marginLeft: `${Math.min(level - 1, 8) * 22}px` }, children:
				(0, jsx.jsxs)("div", { className: "dac-lineage-row", children: [node.children.length > 0 && (0, jsx.jsx)("button", { type: "button", onClick: () => onToggle(node.id), children: collapsed.has(node.id) ? "+" : "−" }), (0, jsx.jsx)("strong", { title: node.id, children: node.title ?? node.id }), (0, jsx.jsx)("span", { className: `dac-trash-status dac-lineage-${node.status}`, style: { borderRadius: "8px", whiteSpace: "nowrap", flexShrink: 0 }, children: t(`lineage.status.${node.status}`) }), (0, jsx.jsx)("small", { children: `${node.id} · ${t(`lineage.origin.${node.origin ?? "session"}`)} · d${node.delegationDepth ?? 0} · ${node.children.length} · ${formatDate(t, node.createdAt)}` })] })
			}, node.id)) });
		}
