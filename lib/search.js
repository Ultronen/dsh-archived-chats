...
function imageAttachment(block) {
  ...
  const displayName = typeof ref.name === 'string'
    ...
    : '';
  return {
    attachmentId: boundedText(ref.attachmentId),
...
function projectBlock(block) {
  ...
  switch (block.type) {
    ...
    case 'tool-call': {
      const name = boundedText(typeof block.name === 'string' && block.name !== '' ? block.name : 'unknown');
      ...
      return {
        ...
        text: argumentsText,
        isError: false,
        callId: typeof block.id === 'string' && block.id !== '' ? boundedText(block.id) : null,
        name,
        argumentsText,
      };
    }
    case 'tool-result': {
      ...
      return {
        kind: 'tool-result',
        label: typeof block.toolCallId === 'string' ? boundedText(block.toolCallId) : null,
        text,
        isError: block.isError === true,
        toolCallId: typeof block.toolCallId === 'string' && block.toolCallId !== '' ? boundedText(block.toolCallId) : null,
      };
    }
    default:
