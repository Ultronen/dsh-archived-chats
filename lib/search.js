...
function metadataText(value) {
  return typeof value === 'string' && value !== ''
    ? boundedText(value, PREVIEW_LIMITS.maxIdentityCodePoints)
    : null;
}

function sourceMetadata(source) {
  const kind = metadataText(source?.kind);
  const sourcePlugin = kind === 'plugin' ? metadataText(source?.plugin) : null;
  let sourceLabel = sourcePlugin ?? kind;
  if (kind === 'skill-invocation') sourceLabel = metadataText(source?.name) ?? kind;
  const entries = kind === 'agent-instructions' ? source?.changes
    : kind === 'session-reference' ? source?.references : null;
  if (Array.isArray(entries)) {
    const labels = new Set();
    let length = 0;
    for (const entry of entries) {
      const label = metadataText(kind === 'agent-instructions' ? entry?.path : entry?.label);
      if (label !== null && !labels.has(label)) {
        labels.add(label);
        length += codePoints(label).length + 2;
        if (length >= PREVIEW_LIMITS.maxIdentityCodePoints) break;
      }
    }
    if (labels.size > 0) sourceLabel = metadataText([...labels].join(', '));
  }
  return {
    source: kind,
    sourcePlugin,
    sourceLabel,
    sourceForm: ['instructions', 'catalog', 'snapshot', 'notice', 'relay', 'recall'].includes(source?.form)
      ? source.form : null,
  };
}
...
/** Retain only system positions and replacement endpoints, never replay ordinary messages. */
function advancePreviewSystem(state, event) {
  const append = isAppendSurfaceEvent(event);
  const replacement = event.surfaceOp?.op === 'replace'
    && ['system/message', 'user/message', 'assistant/message', 'tool/result'].includes(event.type);
  if (event.type !== 'system/message' && !replacement || !append && !replacement) return false;
  const update = append && state.nodes.some((node) => node.hasText);
  if (state.uncertain) return update;
  const seq = coordinate(event.seq);
  state.firstSeq ??= seq;
  let position = seq;
  if (replacement) {
    const start = coordinate(event.surfaceOp.startSeq);
    const end = coordinate(event.surfaceOp.endSeq);
    const unknown = (endpoint) => endpoint === null
      || endpoint < state.firstSeq && !state.replacements.has(endpoint);
    if (unknown(start) || unknown(end)) state.uncertain = true;
    else {
      position = state.replacements.get(start) ?? start;
      const endPosition = state.replacements.get(end) ?? end;
      if (endPosition < position) state.uncertain = true;
      else {
        state.nodes = state.nodes.filter((node) => node.position < position || node.position > endPosition);
        state.replacements = new Map([...state.replacements].filter(([, value]) => value < position || value > endPosition));
        state.replacements.set(seq, position);
      }
    }
  }
  if (seq === null || state.nodes.length >= PREVIEW_LIMITS.maxProjectedMessages
    || state.replacements.size > PREVIEW_LIMITS.maxProjectedMessages) state.uncertain = true;
  if (state.uncertain) {
    state.nodes = [];
  ...
  if (event.type === 'system/message') {
    ...
    state.nodes.push({ seq, position, hasText: content.some((block) => block.text !== ''), message: { ...message, content } });
    state.nodes.sort((left, right) => left.position - right.position);
  }
  state.effective = state.nodes.findLast((node) => node.hasText) ?? null;
  return update;
}

function previewPromptAnchor(seq, turn, previous, initial) {
  if (turn === null || turn.step === null || previous === null && !initial) return seq;
  if (previous?.key === turn.value.key && previous.step === turn.step) return seq;
  return turn.step === 1 ? turn.value.startSeq : turn.stepStartSeq ?? seq;
}

function startPreviewTurn(event) {
  const startSeq = coordinate(event.seq);
  const number = coordinate(event.data?.turn);
  if (startSeq === null || number === null) return null;
  return {
    number, step: null, stepStartSeq: null, latestStep: null, previousSeq: startSeq, complete: true, answer: null,
    rows: [], replyCounts: new Map(),
    value: {
      key: `turn:${startSeq}`, startSeq, endSeq: null, answerSeq: null, processStartSeq: null,
      toolCallCount: 0, messageCount: 0, subagentCount: 0, endReason: null, rowCount: 0,
    },
  };
}

function finishPreviewTurn(turn) {
  if (turn === null) return;
  const value = turn.value;
  const answer = turn.complete && turn.step === null && value.endSeq !== null
    && turn.answer?.step === turn.latestStep ? turn.answer : null;
  value.answerSeq = answer?.seq ?? null;
  value.messageCount = [...turn.replyCounts].reduce((total, [step, count]) => (
    answer === null || step < answer.step ? total + count : total
  ), 0);
  value.processStartSeq = turn.rows.find((row) => row.role !== 'user' && row.role !== 'system'
    && (answer === null || row.seq < answer.seq || row.seq === answer.seq
      && row.segments.some((segment) => segment.kind === 'reasoning' && segment.text.trim() !== '')))?.seq ?? null;
  value.rowCount = turn.rows.length;
  Object.freeze(value);
}

/** Project the historical transcript, retaining bounded native turn and source facts. */
export function projectArchivedMessages(events) {
  const messages = [];
  let turn = null;
  const system = { firstSeq: null, nodes: [], replacements: new Map(), effective: null, uncertain: false };
  let previousRequest = null;
  let presentedSystem = null;
  for (let index = 0; index < (Array.isArray(events) ? events.length : 0); index += 1) {
    const event = events[index];
    if (event === null || typeof event !== 'object') continue;
    const data = event.data ?? {};
    if (event.type === 'turn/start') {
      finishPreviewTurn(turn);
      turn = startPreviewTurn(event);
      continue;
    }
    if (turn !== null) {
      if (coordinate(event.seq) !== turn.previousSeq + 1) turn.complete = false;
      turn.previousSeq = coordinate(event.seq);
    }
    const explicitTurn = coordinate(data.turn);
    const ownsTurn = turn !== null && data.turn !== null
      && (explicitTurn === null || explicitTurn === turn.number);
    if (turn !== null && explicitTurn !== null && explicitTurn !== turn.number) turn.complete = false;
    if (event.type === 'step/start') {
      if (ownsTurn) {
        const step = coordinate(data.step);
        if (turn.step !== null || step === null || turn.latestStep !== null && step <= turn.latestStep) turn.complete = false;
        turn.step = step;
        turn.stepStartSeq = coordinate(event.seq);
        turn.latestStep = step;
        turn.answer = null;
      }
      continue;
    }
    if (event.type === 'step/end') {
      if (ownsTurn) {
        if (turn.step === null || turn.step !== coordinate(data.step)) turn.complete = false;
        turn.step = null;
      }
      continue;
    }
    if (event.type === 'turn/end') {
      if (ownsTurn && explicitTurn === turn.number) {
        turn.value.endSeq = coordinate(event.seq);
        turn.value.endReason = metadataText(data.reason?.kind);
        finishPreviewTurn(turn);
        turn = null;
      }
      continue;
    }
    if (event.type === 'tool/call' && ownsTurn) {
      const isSubagent = typeof data.name === 'string' && (data.name === 'subagent' || data.name.startsWith('subagent_'));
      turn.value[isSubagent ? 'subagentCount' : 'toolCallCount'] += 1;
    }
    const replacementPrompt = event.type === 'system/message' && event.surfaceOp?.op === 'replace';
    const appendPromptUpdate = advancePreviewSystem(system, event);
    const seq = Number.isFinite(event.seq) ? event.seq : index;
    const step = coordinate(data.step) ?? (ownsTurn ? turn.step : null);
    let anchorSeq = seq;
    let message;
    const systemPromptUpdate = event.type === 'system/message' && (replacementPrompt || appendPromptUpdate);
    if (event.type === 'request/header') {
      const current = system.effective;
      const key = ownsTurn ? turn.value.key : null;
      const shown = current !== null && presentedSystem?.seq === current.seq
        && (previousRequest === null || presentedSystem.key === key && presentedSystem.step === step);
      const showPrompt = current !== null && !shown && (previousRequest === null || data.reason !== 'change'
        || data.startsSeries === true || previousRequest.systemSeq !== current.seq);
      anchorSeq = previewPromptAnchor(seq, ownsTurn ? turn : null, previousRequest, data.reason === 'initial');
      previousRequest = { key, step, systemSeq: current?.seq ?? null };
      presentedSystem = null;
      if (!showPrompt) continue;
      message = current.message;
    } else {
      if (!isAppendSurfaceEvent(event) && !replacementPrompt) continue;
      message = deriveEventMessage(event);
      if (event.type === 'system/message' && !systemPromptUpdate) {
        anchorSeq = previewPromptAnchor(seq, ownsTurn ? turn : null, null, true);
      }
    }
    if (message === null) continue;
    if (messages.length >= PREVIEW_LIMITS.maxProjectedMessages) {
      if (ownsTurn) turn.complete = false;
      continue;
    }
    ...
    const row = {
      ...
      ...sourceMetadata(message?.source),
      turn: ownsTurn ? turn.value : null,
      step,
      systemPromptUpdate,
      segments,
      searchable,
    ...
    messages.push(row);
    if (event.type === 'system/message' && system.effective?.seq === seq) {
      presentedSystem = { seq, key: row.turn?.key ?? null, step };
    }
    if (ownsTurn) {
      turn.rows.push(row);
      if (event.type === 'assistant/message') {
        const reply = replyBearing(content);
        if (reply) turn.replyCounts.set(step, (turn.replyCounts.get(step) ?? 0) + 1);
        if (step === null || turn.step !== step) turn.complete = false;
        turn.answer = reply && !content.some((block) => block?.type === 'tool-call') ? { seq: row.seq, step } : null;
      }
    }
  }
  finishPreviewTurn(turn);
...
function projectedCodePoints(messages) {
  ...
  for (const message of messages) {
    for (const field of ['source', 'sourcePlugin', 'sourceLabel', 'sourceForm']) total += codePoints(message[field]).length;
    if (message.turn !== undefined && message.turn !== null) total += codePoints(JSON.stringify(message.turn)).length;
 ...
 * Only needed when NFKC changed the text length (ﬁ → fi, ⑴ → (1), ㎡ → m2):
