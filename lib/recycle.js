...
async function attachmentData(item) {
  ...
  if (!(data instanceof Uint8Array)) throw error('snapshot-attachment-invalid', 'snapshot attachment bytes are unavailable');
  return data;
}
...
export function createRecycleService({
  ...
  async function descriptor(id) {
    ...
    if (typeof persistence.readTitle === 'function') {
      try { title = await persistence.readTitle(id) ?? title; }
  ...
  async function stableRevision(id) {
    if (typeof persistence.listSnapshots !== 'function') return null;
    try {
      const snapshots = await persistence.listSnapshots();
      const item = Array.isArray(snapshots)
        ? snapshots.find((entry) => String(entry?.header?.id ?? entry?.id) === id)
        : undefined;
      return typeof item?.revision === 'string' && item.revision !== '' ? item.revision : null;
    } catch { return null; }
  }
  ...
  async function moveOne(id) {
    return lifecycle.run(async () => {
      ...
      const revisionBefore = await stableRevision(id);
      const reusable = revisionBefore !== null && typeof snapshotStore.findRevision === 'function'
        ? await snapshotStore.findRevision(id, revisionBefore)
        : null;
      ...
      let snapshot;
      let published = false;
      const revisionAfter = await stableRevision(id);
      if (reusable !== null && revisionAfter === revisionBefore) {
        snapshot = {
          ...reusable,
          bytes: reusable.bytes ?? reusable.totalBytes,
        };
      } else {
        try {
      ...
      if (!(registry.archivedSessionIds ?? []).map(String).includes(id) || (await loadedTrash()).has(id)) {
        if (published) await removeNewSnapshot(snapshot.snapshotId, prior);
        throw error('operation-cancelled', 'archive ownership changed', 409);
      }
  ...
  async function restoreOriginal(id, record) {
    ...
    const warnings = [];
    const shouldArchive = record.wasArchived && !originalArchiveIds.includes(id);
    if (shouldArchive) {
    ...
    const shouldAttach = workspace !== null && !originallyAttached
      && typeof workspace.attachSession === 'function' && typeof workspace.detachSession === 'function';
    if (record.workspace?.id !== null && record.workspace?.id !== undefined
      && (workspace === null || (!originallyAttached && !shouldAttach))) {
      warnings.push({ id, reason: 'workspace-unresolved' });
    }
    const shouldSetMetadata = metadata.entries?.[id] === undefined && (record.tags.length > 0 || record.note !== '');
    let registryAttempted = false;
    let attachmentAttempted = false;
    let metadataAttempted = false;
    let trashRemovalAttempted = false;
    ...
    try {
      ...
      if (trashRemovalAttempted) {
        try {
          const current = typeof trashStore.get === 'function' ? await trashStore.get(id) : null;
          if (current === null) await trashStore.put(record);
        } catch (rollbackError) { rollbackErrors.push(rollbackError); }
      }
      if (metadataAttempted) {
        try {
          const prior = metadata.entries?.[id];
          if (prior === undefined) await metadataStore.remove([id]);
          else await metadataStore.set(id, { tags: prior.tags, note: prior.note });
        } catch (rollbackError) { rollbackErrors.push(rollbackError); }
      }
      if (attachmentAttempted && sessionIds(workspace).includes(id)) {
        try { await workspace.detachSession(id); } catch (rollbackError) { rollbackErrors.push(rollbackError); }
      }
      if (registryAttempted
        && JSON.stringify((registry.archivedSessionIds ?? []).map(String)) !== JSON.stringify(originalArchiveIds)) {
        try { await registry.setState({ ...registry.state, archivedSessionIds: originalArchiveIds }); }
        catch (rollbackError) { rollbackErrors.push(rollbackError); }
      }
      if (rollbackErrors.length > 0) {
        throw error('restore-rollback-failed', 'recycle restore rollback failed', 500, { cause: stableCode(rollbackErrors[0]) });
      }
      throw cause;
    }
  }
  ...
  async function restoreSnapshot(id, record) {
    ...
    // Legacy snapshots do not carry a trustworthy inherited cut. Even a v2
    // zero cut is meaningful for a seeded header, so this writer must refuse
    // every seeded source instead of guessing or flattening its boundary.
    if (meta.isSeeded === true || checked.record.source.inheritedEventCount > 0) {
      throw error('snapshot-restore-unsupported', 'this writer cannot preserve the inherited fork boundary', 501);
    }
    ...
    if (typeof location?.path !== 'string' || !isAbsolute(location.path)
      || basename(dirname(location.path)) !== id) {
    ...
    const before = await resolveExclusiveSessionDirectory(persistence, id);
    if (before.status !== 'missing') throw error('id-conflict', 'session id already exists', 409);
    const canonicalDestination = join(await realpath(dirname(sessionDirectory)), basename(sessionDirectory));
    let destinationExists = true;
    try { await lstat(canonicalDestination); }
    catch (cause) {
      if (cause?.code === 'ENOENT') destinationExists = false;
      else throw error('session-location-unavailable', 'restore destination cannot be inspected', 503);
    }
    if (destinationExists) throw error('id-conflict', 'session destination already exists', 409);
    ...
    try {
      ...
      createStarted = true;
      try { await persistence.createExclusive(structuredClone(meta)); }
      catch (cause) {
        if (['EEXIST', 'id-conflict', 'SESSION_ALREADY_EXISTS'].includes(cause?.code)) {
          createStarted = false;
          throw error('id-conflict', 'session id already exists', 409);
        }
        throw cause;
      }
      const owned = await resolveExclusiveSessionDirectory(persistence, id);
      if (owned.status !== 'present' || owned.sessionDirectory !== canonicalDestination) {
        throw error('snapshot-restore-unsupported', 'created session ownership cannot be verified', 501);
      }
      createOwned = true;
      ...
      if (createOwned) {
        try {
          const current = await resolveExclusiveSessionDirectory(persistence, id);
          if (current.status === 'present' && current.sessionDirectory !== canonicalDestination) {
            throw error('snapshot-restore-rollback-incomplete', 'owned restore destination changed; destination was preserved', 500);
          }
          if (current.status === 'present') {
            if (typeof persistence.removeSession === 'function') await persistence.removeSession(id);
            else await rm(current.sessionDirectory, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 });
          }
        } catch (rollbackError) { rollbackErrors.push(rollbackError); }
      }
      if (createStarted && !createOwned) {
        throw error('snapshot-restore-rollback-incomplete', 'snapshot restore ownership is uncertain; destination was preserved', 500, {
          cause: stableCode(cause),
        });
      }
      if (rollbackErrors.length > 0) {
  ...
  async function purgeOne(id, expected, beforePurge, lockHeld = false) {
    const commit = async () => {
      if (typeof purgePhysical !== 'function' || typeof verifyPurgeScope !== 'function'
        || typeof persistence.locate !== 'function') {
        throw error('purge-unsupported', 'physical purge is unavailable', 501);
      }
      ...
      // Order matters. Capabilities were checked before reading or changing the
      // record; snapshots are removed next, then the session log — the irreversible step. Anything
    ...
    for (const id of uniqueIds(ids)) {
      try {
        await lifecycle.run(async () => {
          const existing = (await loadedTrash()).get(id);
          if (existing !== undefined) throw error('session-in-trash', 'session is in trash', 409);
          if (typeof purgePhysical !== 'function' || typeof verifyPurgeScope !== 'function'
            || typeof persistence.locate !== 'function') {
            throw error('purge-unsupported', 'physical purge is unavailable', 501);
          }
          ...
          // Commit irreversible intent before touching snapshots or the live
          // session. Startup and automatic retry can finish even after the
          // original log/header has already gone. No recoverable copy is made.
          await trashStore.put({
            sessionId: id, state: 'purge-pending', trashedAt: at, purgeRequestedAt: at,
            ...source.archive, snapshotId: null, snapshotBytes: 0,
            snapshotAttachmentCount: 0, liveDisposition: 'cold',
          });
          invalidate?.([id]);
          await purgeOne(id, undefined, undefined, true);
        });
        deleted.push(id);
      } catch (cause) {
        failed.push(failure(id, cause));
        if ((await trashStore.get(id))?.state === 'purge-pending') pending.push(id);
      }
    }
    return { deleted, pending, failed };
  }
  ...
  async function purge(ids, options = {}) {
    ...
    const failed = [];
    for (const id of uniqueIds(ids)) {
      try { purged.push(await purgeOne(id, options.expected?.sessionId === id ? options.expected : undefined, options.beforePurge)); }
      catch (cause) { failed.push(failure(id, cause)); }
    }
    return { purged, failed };
  }

  async function empty(targets) {
    if (!Array.isArray(targets) || targets.length === 0) {
      throw error('trashTargets-required', 'captured recycle targets are required', 400);
    }
    const purged = [];
    const failed = [];
    const seen = new Set();
    for (const expected of targets) {
      const id = expected?.sessionId;
      if (typeof id !== 'string' || id === '' || seen.has(id)
        || !['trashed', 'degraded'].includes(expected?.state)
        || typeof expected?.trashedAt !== 'string' || expected.trashedAt === ''
        || !(typeof expected?.snapshotId === 'string' || expected?.snapshotId === null)
        || !Number.isFinite(expected?.bytes) || expected.bytes < 0) {
        throw error('trashTargets-invalid', 'captured recycle targets are invalid', 400);
      }
      seen.add(id);
    }
    for (const expected of targets) {
      const id = expected.sessionId;
      try { purged.push(await purgeOne(id, expected)); }
      catch (cause) { failed.push(failure(id, cause)); }
    }
    return { purged, failed };
  }
  ...
  async function recoverStartup({ legacyPendingPath } = {}) {
    ...
    for (const record of records.values()) {
      ...
      if (record.snapshotId === null || !validSnapshots.has(record.snapshotId)) {
        await lifecycle.run(async () => {
          const current = await trashStore.get(record.sessionId);
          if (current === null || current.state === 'purge-pending') return;
          if (current.snapshotId === null || !validSnapshots.has(current.snapshotId)) {
            await trashStore.markDegraded(current.sessionId, { snapshotId: current.snapshotId });
          }
        });
      }
    }
    ...
    // Historical versions could leave published snapshots that were never
    // referenced by the durable recycle catalog. Re-read both authorities
    // after pending migration because migration may create a protection
    // snapshot which must survive this startup pass.
    if (typeof snapshotStore.inventory === 'function') {
      await lifecycle.run(async () => {
        const currentRecords = await loadedTrash();
        const protectedIds = new Set(
          [...currentRecords.values()]
            .map((record) => record?.snapshotId)
            .filter((snapshotId) => typeof snapshotId === 'string' && snapshotId !== ''),
        );
        const inventory = await snapshotStore.inventory();
        const candidates = [...(inventory?.valid ?? []), ...(inventory?.degraded ?? [])]
          .map((item) => item?.snapshotId)
          .filter((snapshotId) => typeof snapshotId === 'string' && snapshotId !== '' && !protectedIds.has(snapshotId));
        for (const snapshotId of new Set(candidates)) {
          try {
            await snapshotStore.remove(snapshotId);
          } catch (cause) {
            logger?.warn?.('archived-chats: legacy snapshot cleanup failed: ' + stableCode(cause, 'snapshot-cleanup-failed'));
          }
        }
      });
    }
  ...
  return Object.freeze({ move, restore, purge, deleteArchived, empty, recoverStartup, list, summary });
}
