...
function validateRecord(value, manifest) {
  if (!exactKeys(value, ['format', 'version', 'archive', 'source', 'attachments'])
    || value.format !== SESSION_FORMAT || ![1, 2].includes(value.version) || !plainObject(value.archive) || !plainObject(value.source)
    || !exactKeys(value.source, value.version === 2 ? ['meta', 'events', 'inheritedEventCount'] : ['meta', 'events']) || !Array.isArray(value.source.events)
  ...
  if (value.version === 2) validateInheritedBoundary(value.source);
...
function validateInheritedBoundary(source) {
  const count = source.inheritedEventCount;
  if (!Number.isSafeInteger(count) || count < 0 || count > source.events.length
    || typeof source.meta?.isSeeded !== 'boolean' || (!source.meta.isSeeded && count !== 0)) {
    throw failure('snapshot-schema-invalid', 'snapshot inherited boundary is invalid');
  }
}
  ...
  async function capture({ sessionId, archive, liveDisposition }) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < SNAPSHOT_LIMITS.maxRevisionAttempts; attempt += 1) {
        ...
        let inspected;
        try {
          inspected = typeof persistence.readSession === 'function'
            ? await persistence.readSession(sessionId) : await persistence.inspect(sessionId);
        } catch (cause) {
          if (cause?.code === 'session-inspection-unsupported') throw failure(cause.code, 'snapshot source format is unsupported', 501);
          if (cause?.name === 'SessionPersistenceNotFoundError'
            || ['ENOENT', 'session-not-found', 'SESSION_NOT_FOUND'].includes(cause?.code)) {
            throw failure('snapshot-source-missing', 'snapshot source is missing', 404);
          }
          throw failure('snapshot-source-unreadable', 'snapshot source could not be read');
        }
        if (!plainObject(inspected) || !plainObject(inspected.meta) || inspected.meta.id !== sessionId || !Array.isArray(inspected.events)) {
          throw failure('snapshot-source-missing', 'snapshot source is missing', 404);
        }
        const preservesBoundary = Object.hasOwn(inspected, 'inheritedEventCount');
        if (preservesBoundary) validateInheritedBoundary(inspected);
