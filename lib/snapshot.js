...
const REFERENCE_KEYS = [...REQUIRED_REFERENCE_KEYS, 'name'];
...
function sameReference(left, right) {
  try {
    ...
    return REFERENCE_KEYS.every((key) => a[key] === b[key]);
  } catch { return false; }
}
...
function exactReferenceKeys(value, extraKeys = []) {
  ...
  return expected.every((key) => Object.hasOwn(value, key))
    && keys.every((key) => expected.includes(key) || key === 'name')
    && keys.length === expected.length + (Object.hasOwn(value, 'name') ? 1 : 0);
}
...
export function createSnapshotStore({ root, persistence, attachments, now = () => new Date(), uuid = randomUUID }) {
  ...
  async function recover() {
    ...
    for (const entry of await readdir(staging)) await rm(resolve(staging, entry), { recursive: true, force: true });
    const valid = [];
    const degraded = [];
  ...
  async function latestFor(sessionId) {
    ...
    return (await recover()).latestBySession.get(sessionId) ?? null;
  }
  ...
  return Object.freeze({ capture, validate, latestFor, recover, remove, removeForSession });
}
