...
function parseLines(bytes, maxBytes, maxLines, maxLineBytes, losses, initialSessionId) {
  ...
  const consume = (raw, number) => {
    ...
    if (recordType(record) === 'session_meta') {
      const metadataId = sessionIdFor(record, '');
      if (!isObject(record.payload) || !metadataId) {
        losses.push(loss('record-invalid', number, id, {
          recordType: 'session_meta', reason: 'session-meta-payload-invalid', record: clone(record),
        }));
      }
...
export function exportCodexJsonl(session, options = {}) {
  ...
  const usedCallIds = new Set(messages
    .map((message) => boundedString(message?.toolCallId, 240))
    .filter(Boolean));
  let generatedCall = 0;
  const callIdFor = (message) => {
    ...
    let generated;
    do generated = `call-${++generatedCall}`; while (usedCallIds.has(generated));
  ...
  for (const message of messages) {
    ...
    if (role === 'user' || role === 'assistant' || role === 'system') {
      if (message.toolCall && isObject(message.toolCall)) {
        const callId = callIdFor(message);
      ...
      if (content !== '' || !message.toolCall) {
        records.push({ type: 'response_item', payload: { type: 'message', role, content: [{ type: role === 'user' ? 'input_text' : 'output_text', text: content }] } });
      }
    } else if (role === 'tool') {
      const callId = callIdFor(message);
