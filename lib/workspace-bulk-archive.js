...
export function createWorkspaceBulkArchiveService({ registry, sessions, historyService, lifecycle, now = () => new Date(), secret } = {}) {
  if (registry === null || typeof registry !== 'object'
    ...
    || !Object.prototype.hasOwnProperty.call(registry, 'archivedSessionIds') && !('archivedSessionIds' in registry)
  ...
  function archivedIds() {
    return new Set(asIds(registry.archivedSessionIds).map((id) => String(id)));
  }
  ...
  async function execute(token, nonce) {
    ...
    if (confirmation.consumed) {
      throw failure('workspace-archive-confirmation-replayed', 'workspace archive confirmation was already used', 409);
    }
      ...
      try {
        await lifecycle.run(async () => {
          ...
          if (workspace === null || !asIds(workspace.sessionIds).some((entry) => safeSession(entry)?.id === sessionId)) {
            result.skipped.push({ id: sessionId, reason: 'session-workspace-changed' });
            return;
          }
          if (sessions.get(sessionId) !== undefined) {
            result.skipped.push({ id: sessionId, reason: 'session-live' });
            return;
          }
          if (archivedIds().has(sessionId)) {
            result.skipped.push({ id: sessionId, reason: 'session-archived' });
            return;
          }
          try {
            await registry.archiveSession.call(registry, sessionId);
          } catch {
            result.failed.push({ id: sessionId, reason: 'archive-failed' });
            return;
          }
          if (!archivedIds().has(sessionId)) {
            result.failed.push({ id: sessionId, reason: 'archive-uncommitted' });
            return;
          }
          result.archived.push(sessionId);
          try {
            await historyService.captureArchived(sessionId, { lockHeld: true });
            result.snapshots.push({ id: sessionId, status: 'captured' });
          } catch {
            result.snapshots.push({ id: sessionId, status: 'snapshot-failed' });
          }
        });
      } catch {
