export class LineageError extends Error {
  constructor(code, message, status = 500) {
    super(message);
    this.name = 'LineageError';
    this.code = code;
    this.status = status;
  }
}

const invalid = () => new LineageError('lineage-input-invalid', 'lineage input is invalid', 503);

function sessionIds(workspace) {
  if (Array.isArray(workspace?.sessionIds)) return workspace.sessionIds.map(String);
  if (workspace?.sessionIds instanceof Set) return [...workspace.sessionIds].map(String);
  return [];
}

function sortNodes(left, right) {
  const leftTime = Number.isSafeInteger(left.createdAt) ? left.createdAt : Number.MAX_SAFE_INTEGER;
  const rightTime = Number.isSafeInteger(right.createdAt) ? right.createdAt : Number.MAX_SAFE_INTEGER;
  return leftTime - rightTime || left.id.localeCompare(right.id);
}

function cloneNode(node) {
  return {
    id: node.id,
    parentSession: node.parentSession,
    seedLength: node.seedLength,
    origin: node.origin,
    delegationDepth: node.delegationDepth,
    title: node.title,
    createdAt: node.createdAt,
    workspace: { ...node.workspace },
    status: node.status,
    children: [],
  };
}

function cloneForest(roots, included = null) {
  const keep = (node) => included === null || included.has(node.id);
  const sources = [...roots].filter(keep).sort(sortNodes);
  const output = sources.map(cloneNode);
  const stack = sources.map((source, index) => ({ source, target: output[index] }));
  while (stack.length > 0) {
    const { source, target } = stack.pop();
    const children = [...source.children].filter(keep).sort(sortNodes);
    target.children = children.map(cloneNode);
    for (let index = children.length - 1; index >= 0; index -= 1) {
      stack.push({ source: children[index], target: target.children[index] });
    }
  }
  return output;
}

export function projectLineage({
  headers,
  archivedIds = [],
  trashRecords = new Map(),
  workspaces = [],
  titles = new Map(),
  maxNodes = 5000,
  focusIds,
}) {
  if (!Array.isArray(headers) || !Number.isSafeInteger(maxNodes) || maxNodes < 1
    || !(trashRecords instanceof Map) || !(titles instanceof Map) || !Array.isArray(workspaces)
    || (focusIds !== undefined && (!Array.isArray(focusIds)
      || focusIds.some((id) => typeof id !== 'string' || id === '')))) throw invalid();

  /**
   * The node limit bounds the graph this projection RETURNS, not how many
   * sessions the Host happens to store. `focusIds` already narrows the output to
   * the archived and recycled chats plus the context that explains them, so a
   * store of 100,000 sessions with 25 archived projects 25 nodes. Counting the
   * input instead refused that outright, and the measured cost of the wide input
   * it was guarding against is milliseconds.
   */
  const withinLimit = (count) => {
    if (count > maxNodes) throw new LineageError('lineage-limit-exceeded', 'lineage exceeds the node limit', 413);
    return count;
  };

  const archived = new Set((Array.isArray(archivedIds) ? archivedIds : []).map(String));
  const trashed = new Set([...trashRecords.keys()].map(String));
  const workspaceBySession = new Map();
  for (const workspace of workspaces) {
    if (workspace === null || typeof workspace !== 'object') continue;
    const safe = {
      id: typeof workspace.id === 'string' ? workspace.id : null,
      title: typeof workspace.title === 'string' ? workspace.title : null,
    };
    for (const id of sessionIds(workspace)) if (!workspaceBySession.has(id)) workspaceBySession.set(id, safe);
  }

  const nodes = new Map();
  const order = [];
  for (const header of headers) {
    // Headers are the Host's data, so every field here is a forward-compatibility
    // surface: a new `origin` value, an absent timestamp, a changed numeric type.
    // Each one degrades that single node's detail to a value the public
    // LineageNode already allows. Only an unusable identity drops the row, and a
    // duplicate id keeps the first. One unrecognized session must never cost the
    // whole graph — this projection was the only surface that failed that way.
    if (header === null || typeof header !== 'object' || Array.isArray(header)) continue;
    if (typeof header.id !== 'string' || header.id === '' || nodes.has(header.id)) continue;
    const id = header.id;
    const cachedTitle = titles.get(id);
    const node = {
      id,
      parentSession: typeof header.parentSession === 'string' && header.parentSession !== '' ? header.parentSession : null,
      seedLength: Number.isSafeInteger(header.seedLength) && header.seedLength >= 0 ? header.seedLength : null,
      origin: header.origin === 'subagent' ? 'subagent' : null,
      delegationDepth: Number.isSafeInteger(header.delegationDepth) && header.delegationDepth >= 0 ? header.delegationDepth : 0,
      title: typeof cachedTitle === 'string' ? cachedTitle : (typeof header.title === 'string' ? header.title : null),
      createdAt: Number.isSafeInteger(header.createdAt) && header.createdAt >= 0 ? header.createdAt : null,
      workspace: { ...(workspaceBySession.get(id) ?? { id: null, title: null }) },
      status: trashed.has(id) ? 'trash' : archived.has(id) ? 'archived' : 'active',
      children: [],
    };
    nodes.set(id, node);
    order.push(id);
  }

  for (const [key, record] of trashRecords) {
    const id = String(key);
    if (nodes.has(id)) continue;
    if (record === null || typeof record !== 'object' || String(record.sessionId) !== id) continue;
    const workspace = record.workspace;
    nodes.set(id, {
      id,
      parentSession: null,
      seedLength: null,
      origin: record.origin === 'subagent' ? 'subagent' : null,
      delegationDepth: 0,
      title: typeof titles.get(id) === 'string'
        ? titles.get(id)
        : typeof record.title === 'string' ? record.title : null,
      createdAt: Number.isSafeInteger(record.createdAt) && record.createdAt >= 0 ? record.createdAt : null,
      workspace: {
        id: typeof workspace?.id === 'string' ? workspace.id : null,
        title: typeof workspace?.title === 'string' ? workspace.title : null,
      },
      status: 'trash',
      children: [],
    });
    order.push(id);
  }

  const diagnostics = [];
  const detached = new Set();
  for (const id of order) {
    const node = nodes.get(id);
    if (node.parentSession === id) {
      detached.add(id);
      diagnostics.push({ code: 'self-parent', sessionId: id, relatedId: id });
    }
  }

  const state = new Map();
  const cycleNodes = new Set();
  for (const startId of order) {
    if ((state.get(startId) ?? 0) !== 0) continue;
    const path = [];
    const pathIndex = new Map();
    let id = startId;
    while ((state.get(id) ?? 0) === 0) {
      state.set(id, 1);
      pathIndex.set(id, path.length);
      path.push(id);
      const parent = nodes.get(id).parentSession;
      if (parent === null || parent === id || !nodes.has(parent)) break;
      const parentState = state.get(parent) ?? 0;
      if (parentState === 1) {
        const cycleStart = pathIndex.get(parent);
        if (cycleStart !== undefined) {
          for (let index = cycleStart; index < path.length; index += 1) cycleNodes.add(path[index]);
        }
        break;
      }
      if (parentState === 2) break;
      id = parent;
    }
    for (let index = path.length - 1; index >= 0; index -= 1) state.set(path[index], 2);
  }
  for (const id of order) {
    if (!cycleNodes.has(id)) continue;
    detached.add(id);
    diagnostics.push({ code: 'cycle', sessionId: id, relatedId: nodes.get(id).parentSession });
  }

  const missingNodes = new Map();
  for (const id of order) {
    const node = nodes.get(id);
    const parent = node.parentSession;
    if (parent === null || detached.has(id) || nodes.has(parent)) continue;
    if (!missingNodes.has(parent)) {
      missingNodes.set(parent, {
        id: parent,
        parentSession: null,
        seedLength: null,
        origin: null,
        delegationDepth: 0,
        title: null,
        createdAt: null,
        workspace: { id: null, title: null },
        status: 'missing',
        children: [],
      });
    }
    diagnostics.push({ code: 'missing-parent', sessionId: id, relatedId: parent });
  }

  for (const id of order) {
    const node = nodes.get(id);
    const parent = node.parentSession;
    if (node.origin !== 'subagent' || parent === null || detached.has(id) || !nodes.has(parent)) continue;
    if (node.delegationDepth !== nodes.get(parent).delegationDepth + 1) {
      diagnostics.push({ code: 'delegation-depth-mismatch', sessionId: id, relatedId: parent });
    }
  }

  const roots = [];
  for (const id of order) {
    const node = nodes.get(id);
    const parent = node.parentSession;
    if (parent === null || detached.has(id)) roots.push(node);
    else if (nodes.has(parent)) nodes.get(parent).children.push(node);
    else missingNodes.get(parent).children.push(node);
  }
  roots.push(...missingNodes.values());

  if (focusIds !== undefined) {
    const allNodes = new Map([...nodes, ...missingNodes]);
    const included = new Set();
    for (const id of new Set(focusIds)) {
      const start = allNodes.get(id);
      if (start === undefined) continue;
      included.add(start.id);
      const seen = new Set();
      let current = start;
      while (current.parentSession !== null && !seen.has(current.parentSession)) {
        seen.add(current.parentSession);
        const parent = allNodes.get(current.parentSession);
        if (parent === undefined) break;
        included.add(parent.id);
        current = parent;
      }
    }
    withinLimit(included.size);
    return {
      roots: cloneForest(roots, included),
      diagnostics: diagnostics.filter((item) => included.has(item.sessionId)).map((item) => ({ ...item })),
      nodeCount: [...included].filter((id) => nodes.has(id)).length,
    };
  }

  withinLimit(nodes.size + missingNodes.size);
  return {
    roots: cloneForest(roots),
    diagnostics: diagnostics.map((item) => ({ ...item })),
    nodeCount: nodes.size,
  };
}
