...
export function createHistoryService({
  ...
  function versionFromInventory(inventory, snapshotId) {
    if (typeof snapshotId !== 'string' || snapshotId === '') throw failure('history-snapshot-invalid', 'history snapshot id is required', 400);
    const version = inventory.sessions
      .flatMap((session) => session.versions.map((item) => ({ ...item, sessionId: session.sessionId })))
      .find((item) => item.snapshotId === snapshotId);
    if (version !== undefined) return version;
    if (inventory.degraded.some((item) => item.snapshotId === snapshotId)) {
      throw failure('history-snapshot-degraded', 'history snapshot is degraded', 409);
    }
    throw failure('history-snapshot-missing', 'history snapshot was not found', 404);
  }

  async function authorizedVersion(snapshotId) {
    return versionFromInventory(await list(), snapshotId);
  }

  function mapReadFailure(error) {
    invalidate();
    if (error instanceof HistoryError) return error;
    if (typeof error?.code === 'string' && error.code.startsWith('snapshot-')) {
      return failure('history-snapshot-degraded', 'history snapshot is degraded', 409);
    }
    return error;
  }

  async function preview(snapshotId, window) {
    if (typeof snapshotStore.readHistoryPage !== 'function') throw failure('history-preview-unsupported', 'history preview is unavailable', 501);
    const version = await authorizedVersion(snapshotId);
    try {
      const page = await snapshotStore.readHistoryPage(snapshotId, window);
      return {
        ...page,
        snapshot: {
      ...
      throw mapReadFailure(error);
    }
  }

  async function readImage(snapshotId, reference, signal) {
    if (typeof snapshotStore.readHistoryImage !== 'function') throw failure('history-preview-unsupported', 'history preview is unavailable', 501);
    await authorizedVersion(snapshotId);
    try { return await snapshotStore.readHistoryImage(snapshotId, reference, signal); }
    catch (error) { throw mapReadFailure(error); }
  }
  ...
  async function deleteVersion(snapshotId) {
    ...
    return lifecycle.run(async () => {
      invalidate();
      const version = versionFromInventory(await compute(), snapshotId);
      ...
      invalidate();
      return { deleted: [snapshotId], freedBytes: version.totalBytes };
    });
  }
    ...
    return lifecycle.run(async () => {
      ...
      for (const session of inventory.sessions) {
        for (const version of session.versions) {
          ...
          try {
            await snapshotStore.remove(version.snapshotId);
      ...
      for (const item of inventory.degraded) {
        skipped.push({ snapshotId: item.snapshotId, reason: 'history-snapshot-degraded' });
      }
      invalidate();
  ...
  return Object.freeze({ captureArchived, list, preview, readImage, deleteVersion, clear, invalidate });
}
