...
export function createLegacyPendingStore({ path }) {
  ...
  async function save(ids) {
    ...
    try {
      await writeFile(tempPath, `${JSON.stringify({ ids }, null, 2)}\n`, { encoding: 'utf8', mode: 0o600, flag: 'wx' });
  ...
  async function save(records) {
    ...
    try {
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        let created = false;
        try {
          await writeFile(candidate, text, { encoding: 'utf8', mode: 0o600, flag: 'wx' });
          created = true;
          await chmod(candidate, 0o600);
          break;
        } catch (error) {
          if (created || error?.code !== 'EEXIST') await rm(candidate, { force: true }).catch(() => undefined);
          tempPath = null;
