...
function cloneWorkspace(workspace) {
  ...
  if (workspace.title !== null && typeof workspace.title !== 'string') invalidRecord('workspace.title');
  if (workspace.path !== null && typeof workspace.path !== 'string') invalidRecord('workspace.path');
...
export function normalizeTrashRecord(input, mapKey) {
  ...
  if (!isTimestamp(input.trashedAt)) invalidRecord('trashedAt');
  if (input.purgeRequestedAt !== null && !isTimestamp(input.purgeRequestedAt)) invalidRecord('purgeRequestedAt');
  ...
  if (input.title !== null && typeof input.title !== 'string') invalidRecord('title');
  if (input.createdAt !== null && (!Number.isSafeInteger(input.createdAt) || input.createdAt < 0)) invalidRecord('createdAt');
  ...
  if (input.metadataUpdatedAt !== null && !isTimestamp(input.metadataUpdatedAt)) invalidRecord('metadataUpdatedAt');
  if (input.state === 'purge-pending' && input.snapshotId === null) {
    // Direct permanent deletion has durable intent, but creates no backup.
    if (input.snapshotBytes !== 0 || input.snapshotAttachmentCount !== 0) invalidRecord('snapshotless purge sizes');
  } else if (input.state === 'trashed' || input.state === 'purge-pending') {
    if (typeof input.snapshotId !== 'string' || !UUID.test(input.snapshotId)) invalidRecord('snapshotId');
  } else if (input.snapshotId !== null && typeof input.snapshotId !== 'string') {
...
export function createLegacyPendingStore({ path }) {
  const filePath = resolve(path);
  const queueKey = `legacy:${filePath}`;

  async function save(ids) {
    await atomicWriteFile(filePath, `${JSON.stringify({ ids }, null, 2)}\n`, { encoding: 'utf8' });
  }

  function mutate(operation) {
    const previous = WRITE_QUEUES.get(queueKey) ?? Promise.resolve();
    const result = previous.then(async () => {
      const loaded = await readLegacyPending(filePath);
      if (loaded.status !== 'ready') throw unavailable('legacy pending store is unreadable');
      return operation([...loaded.ids]);
    });
    const settled = result.catch(() => undefined);
    WRITE_QUEUES.set(queueKey, settled);
    settled.then(() => { if (WRITE_QUEUES.get(queueKey) === settled) WRITE_QUEUES.delete(queueKey); });
    return result;
  }

  return Object.freeze({
    load: () => readLegacyPending(filePath),
    remove(ids) {
...
export function createTrashStore({ path, now = () => new Date() }) {
  ...
  async function save(records) {
    const text = `${JSON.stringify(documentObject(records), null, 2)}\n`;
    await atomicWriteFile(filePath, text, { encoding: 'utf8' });
  }
