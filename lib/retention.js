...
function clonePolicy(value) {
  return {
    ...
    recycleAutoDelete: value.recycleAutoDelete ?? false,
  };
}
...
export function normalizeRetentionPolicy(input) {
  if ((!exactKeys(input, POLICY_KEYS) && !exactKeys(input, POLICY_KEYS.filter((key) => key !== 'recycleAutoDelete')))
    || (Object.hasOwn(input, 'recycleAutoDelete') && typeof input.recycleAutoDelete !== 'boolean')
    || (input.recycleAutoDelete === true && input.recycleMaxAgeDays === null)
...
function parseDocument(text) {
  ...
  if (!exactKeys(value, ['version', 'policy']) || ![1, RETENTION_VERSION].includes(value.version)) throw unavailableStore();
  try {
    if (value.version === 1) {
      const policy = normalizeRetentionPolicy(value.policy);
      return { ...policy, recycleAutoDelete: false };
    }
    if (!exactKeys(value.policy, POLICY_KEYS)) throw invalidPolicy();
    return normalizeRetentionPolicy(value.policy);
  }
  catch { throw unavailableStore(); }
}
