  ...
  async function compute() {
    ...
    for (const item of inventory.valid) {
      if (item === null || typeof item !== 'object'
        || typeof item.snapshotId !== 'string' || item.snapshotId === '') throw authorityUnavailable();
      if (!activeSnapshotIds.has(item.snapshotId)) continue;
      if (item === null || typeof item !== 'object'
    ...
    for (const item of inventory.degraded) {
      if (item === null || typeof item !== 'object'
        || typeof item.snapshotId !== 'string' || item.snapshotId === '') throw authorityUnavailable();
      if (!activeSnapshotIds.has(item.snapshotId)) continue;
      if (typeof item.code !== 'string' || item.code === '') throw authorityUnavailable();
      snapshots.push({
        snapshotId: item.snapshotId,
        status: 'degraded',
        code: item.code,
    ...
    return {
      ...
      summary: {
        ...
        degradedSnapshotCount: snapshots.filter((row) => row.status === 'degraded').length,
      },
      sessions,
      snapshots,
    };
