 ...
 */
async function listArchived(ctx, registry, persistence, titleCache, metadataStore) {
 ...
 */
async function deleteSession(ctx, registry, persistence, id, titleCache, statsService, metadataStore) {
 ...
 */
async function sweepPendingDeletions(ctx, registry, persistence, statsService, metadataStore) {
    let pending;
    try {
    ...
    for (const id of [...pending]) {
        try {
            ...
            await deleteSession(ctx, registry, persistence, id, undefined, statsService, metadataStore);
            pending.delete(id);
...
//#region routes
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService) {
    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/state`,
        handler: async (req, res) => {
            try {
                send(res, 200, await listArchived(ctx, registry, persistence, titleCache, metadataStore));
            } catch (error) {
    ...
    const deleteHandler = (batch) => async (req, res) => {
        ...
        try {
            ...
            for (const id of ids) {
                try {
                    const outcome = await deleteSession(ctx, registry, persistence, id, titleCache, statsService, metadataStore);
...
export function apply(ctx) {
    ...
    const registerWebSurface = () => {
        ...
        registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService);
        ...
        void sweepPendingDeletions(ctx, registry, persistence, statsService, metadataStore)
