...
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, retentionStore, lifecycle, importTokens, restoreAdapter) {
    ...
    const enrichWorkspaceArchivePreview = async (preview) => {
        let headers = [];
        try { headers = await persistence.list(); } catch { /* Safe null fallbacks retain the confirmation. */ }
        const headerById = new Map((Array.isArray(headers) ? headers : []).map((header) => [String(header?.id), header]));
        const sourceSessions = Array.isArray(preview?.sessions) ? preview.sessions : [];
        const sessions = new Array(sourceSessions.length);
        let nextIndex = 0;
        const inspectOne = async () => {
            while (nextIndex < sourceSessions.length) {
                const index = nextIndex;
                nextIndex += 1;
                const session = sourceSessions[index];
            const header = headerById.get(session.id);
            let title = null;
            try {
                const inspected = await persistence.inspect(session.id);
                title = extractTitle(Array.isArray(inspected?.events) ? inspected.events : []) ?? null;
            } catch {
                // An unavailable log stays selectable and deliberately untitled.
            }
                sessions[index] = {
                id: session.id,
                title,
        ...
        await Promise.all(Array.from({ length: Math.min(8, sourceSessions.length) }, inspectOne));
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                send(res, 200, { ok: true, ...(await enrichWorkspaceArchivePreview(await service.preview(body.workspaceId))) });
