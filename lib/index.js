...
//#region routes
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, retentionStore, legacyRecycleStore, lifecycle, importTokens, restoreAdapter) {
    ...
    const regularRecycleService = createRecycleService({
        registry,
        persistence,
    ...
    const legacyRecycleService = createLegacyRecycleService({
        store: legacyRecycleStore,
        historyService,
    ...
    const recycleService = createRecycleHub({
        recycleService: regularRecycleService,
        legacyRecycleService,
    });
    const unifiedTrashStore = createUnifiedTrashStore({ trashStore, legacyRecycleService });
    const retentionService = createRetentionService({
        insightsService,
        retentionStore,
        trashStore: unifiedTrashStore,
        snapshotStore,
        recycleService,
        lifecycle,
    });
...
export function apply(ctx) {
    ...
    const legacyRecycleStore = createLegacyRecycleStore({ path: join(pluginDataRoot, 'legacy-recycle.json') });
    ...
    const registerWebSurface = () => {
        ...
        const { recycleService, retentionService } = registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, retentionStore, legacyRecycleStore, lifecycle, importTokens, restoreAdapter);
