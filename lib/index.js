 ...
 */
async function deleteSession(ctx, registry, persistence, id, titleCache, statsService, metadataStore, pendingStore, lifecycle, requirePending = false) {
    let live = false;
    if (ctx.get('sessions')?.get(id) !== undefined) {
        live = true;
        await lifecycle.run(() => pendingStore.add(id));
