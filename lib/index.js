...
//#region routes
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, lifecycle, importTokens, restoreAdapter) {
    ...
    const recycleService = createRecycleService({
        ...
        purgePhysical: async (id) => {
            ...
            if (outcome !== 'deleted') throw Object.assign(new Error('physical purge was cancelled'), { code: 'purge-cancelled' });
        },
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if ((await trashStore.get(body.sessionId)) !== null) {
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const archivedSessionIds = await lifecycle.run(async () => {
                    if ((await trashStore.get(body.sessionId)) !== null) throw Object.assign(new Error('session is in trash'), { code: 'session-in-trash', status: 409 });
...
export function apply(ctx) {
    ...
    const registerWebSurface = () => {
        ...
        const recycleService = registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, lifecycle, importTokens, restoreAdapter);
