 ...
 *
 *   Direct permanent-delete responses carry { ok, deleted, pending, failed }: `deleted` is
 *   completed permanent deletes; `pending` identifies durable deletion tasks
 *   awaiting retry, including parked live sessions and partial failures.
 *
 ...
 * updates live). Non-permanent delete moves chats into the recycle catalog.
 * Permanent deletion additionally detaches the session from its workspace
 * record and removes the session-log directory from disk after snapshot cleanup.
 *
 * A COLD session can complete permanent deletion in the same request. A LIVE session (opened at least once
 ...
 * conservative behavior: the agent is left parked (cancelled, flushed), with
 * a durable purge-pending record for a confirmed permanent deletion. The boot sweep
 * (`recycleService.recoverStartup`, launched once when the plugin's three
 * services have all bound) retries records carrying durable `purge-pending`
 * intent. Only legacy markers without that intent migrate to recoverable trash.
 * The pending store also brackets the successful live path (recorded before
 * disposal, cleared after all session cleanup), so a crash mid-delete is swept
 * on the next boot instead of leaving a half-deleted ghost. Pending permanent
 * deletions cannot be restored or unarchived. Legacy pending markers without
 * confirmed purge intent still migrate to recoverable trash.
 * After migration, startup removes plugin snapshots not referenced by current
 * recycle records, independently of opt-in Recycle Bin expiry cleanup.
 *
...
import { startRetentionScheduler } from './auto-retention.js';
import { createRetentionService } from './retention-service.js';
import { projectLineage } from './lineage.js';
import { createWorkspaceBulkArchiveService } from './workspace-bulk-archive.js';
import { resolvePersistenceCompat } from './persistence-compat.js';
import { atomicWriteFile } from './durable.js';
import { createAboutRouteHandlers, createAboutService } from './about.js';
...
const aboutService = createAboutService({ version: PLUGIN_VERSION });
...
async function listArchived(ctx, registry, persistence, titleCache, metadataStore, pendingStore, trashStore) {
    ...
    for (const id of archivedIds) {
        ...
        if (title === undefined) {
            try {
                title = typeof persistence.readTitle === 'function'
                    ? await persistence.readTitle(id)
                    : extractTitle((await persistence.inspect(id)).events);
...
function reachabilityError(code, status, reason, sessionIds, message) {
    return Object.assign(new Error(message), { code, status, reason, sessionIds });
}

/**
 * Verify requested archived rows against persistence's authoritative headers.
 * Registry/list rows are intentionally not accepted: they can outlive or omit
 * the stored header that the Host's cold session.list actually evaluates.
 */
async function requireMainListReachability(persistence, registry, ids) {
    const archived = new Set(registry.archivedSessionIds.map(String));
    const targets = [...new Set(ids.map(String))].filter((id) => archived.has(id));
    if (targets.length === 0) return;
    let headers;
    try {
        if (typeof persistence?.list !== 'function') throw new Error('header inventory is unsupported');
        headers = await persistence.list();
    } catch {
        throw reachabilityError(
            'session-reachability-unavailable', 503, 'header-inventory-unavailable', targets,
            'session reachability cannot be verified',
        );
    }
    ...
    for (const header of headers) {
        ...
        else byId.set(id, header);
    }
    const unavailable = targets.filter((id) => !byId.has(id) || ambiguous.has(id));
    if (unavailable.length > 0) {
        throw reachabilityError(
            'session-reachability-unavailable', 503, 'session-header-unavailable', unavailable,
            'session reachability cannot be verified',
        );
    }
    const cwdMissing = targets.filter((id) => byId.get(id).cwd === undefined);
    if (cwdMissing.length > 0) {
        throw reachabilityError(
            'session-main-list-unreachable', 409, 'cwd-missing', cwdMissing,
            'session cannot be published in the main chat list',
        );
    }
    const invalid = targets.filter((id) => typeof byId.get(id).cwd !== 'string' || !isAbsolute(byId.get(id).cwd));
    if (invalid.length > 0) {
        throw reachabilityError(
            'session-reachability-unavailable', 503, 'session-header-invalid', invalid,
            'session reachability cannot be verified',
        );
    }
}

async function requireNoPendingDeletion(pendingStore, ids) {
    let pending;
    try { pending = await pendingStore.load(); }
    catch {
        throw Object.assign(new Error('pending deletion authority is unavailable'), {
            code: 'pending-store-unavailable', status: 503,
        });
    }
    const blocked = [...new Set(ids.map(String))].filter((id) => pending.has(id));
    if (blocked.length > 0) {
        throw Object.assign(new Error('session deletion is pending'), {
            code: 'session-deletion-pending', status: 409, sessionIds: blocked,
        });
    }
}
...
/** Finish a durable purge under the caller's lifecycle lock, after live disposal. */
async function deleteSession(ctx, registry, persistence, id, titleCache, statsService, metadataStore, pendingStore) {
    const commit = async () => {
        ...
        const scope = await resolveExclusiveSessionDirectory(persistence, String(id));
        if (scope.status === 'present') {
            const sessionDirectory = scope.sessionDirectory;
...
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, retentionStore, lifecycle, importTokens, restoreAdapter) {
    const aboutRoutes = createAboutRouteHandlers({ service: aboutService, guard, readBody, send });
    const projectedMessages = createProjectedMessageCache((id) => typeof persistence.readSession === 'function'
        ? persistence.readSession(id) : persistence.inspect(id));
    ...
    let workspaceBulkArchiveService = null;
    let workspaceArchiveUnavailableLogged = false;
    const workspaceArchiveSecret = randomUUID();
    const inspectConversation = async (sessionId) => {
        const live = ctx.get('sessions')?.get(sessionId);
        const inspected = live !== undefined
            ? { meta: live.header, events: typeof live.snapshotEvents === 'function' ? live.snapshotEvents() : null }
            : await persistence.inspect(sessionId);
        const events = inspected?.events;
        if (!Array.isArray(events)) throw new TypeError('session events are unavailable');
        return {
            hasConversation: events.some((event) => event !== null && typeof event === 'object' && event.type === 'turn/start'),
            title: extractTitle(events) ?? null,
            createdAt: Number.isFinite(inspected?.meta?.createdAt) ? inspected.meta.createdAt : null,
        };
    };
    const getWorkspaceBulkArchiveService = () => {
        if (workspaceBulkArchiveService !== null) return workspaceBulkArchiveService;
        try {
            workspaceBulkArchiveService = createWorkspaceBulkArchiveService({
                registry,
            ...
            workspaceArchiveUnavailableLogged = false;
        } catch (error) {
            if (!workspaceArchiveUnavailableLogged) {
                workspaceArchiveUnavailableLogged = true;
                ctx.logger.warn(`archived-chats: workspace archive unavailable: ${String(error?.code ?? error?.name ?? 'workspace-archive-unsupported')}`);
            }
        }
        return workspaceBulkArchiveService;
    };
    ...
    const recycleService = createRecycleService({
        ...
        verifyPurgeScope: typeof persistence.locate === 'function'
            ? (id) => resolveExclusiveSessionDirectory(persistence, String(id))
            : undefined,
        purgePhysical: typeof persistence.locate === 'function' ? async (id) => {
            ...
            const outcome = await deleteSession(ctx, registry, persistence, id, titleCache, statsService, metadataStore, pendingStore);
    ...
    const workspaceArchiveError = (res, error, fallback) => {
    ...
    const workspaceArchiveUnavailable = (res) => send(res, 501, { ok: false, error: 'workspace-archive-unsupported' });

    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/about`,
        handler: aboutRoutes.about,
    }), 'archived-chats: about route');

    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/about/check-updates`,
        handler: aboutRoutes.checkUpdates,
    }), 'archived-chats: about update route');

    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/workspace-archive/workspaces`,
        handler: async (req, res) => {
            if (req.method !== 'GET') { send(res, 405, { ok: false, error: 'method-not-allowed' }); return; }
            const service = getWorkspaceBulkArchiveService();
            if (service === null) { workspaceArchiveUnavailable(res); return; }
            try { send(res, 200, { ok: true, workspaces: await service.listWorkspaces() }); }
            catch (error) { workspaceArchiveError(res, error, 'workspace-archive-workspaces-failed'); }
        },
    }), 'archived-chats: workspace archive workspaces route');

    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/workspace-archive/preview`,
        handler: async (req, res) => {
            ...
            try {
                ...
                if (!exactBody(body, ['workspaceId']) || typeof body.workspaceId !== 'string') {
                    send(res, 400, { ok: false, error: 'workspace-archive-preview-invalid' });
                    return;
                }
                send(res, 200, { ok: true, ...(await service.preview(body.workspaceId)) });
            } catch (error) { workspaceArchiveError(res, error, 'workspace-archive-preview-failed'); }
        },
    }), 'archived-chats: workspace archive preview route');

    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/workspace-archive/apply`,
        handler: async (req, res) => {
            if (!guard(req, res)) return;
            const service = getWorkspaceBulkArchiveService();
            if (service === null) { workspaceArchiveUnavailable(res); return; }
            try {
                ...
                if (!exactBody(body, ['token', 'nonce']) || typeof body.token !== 'string' || typeof body.nonce !== 'string') {
                    send(res, 400, { ok: false, error: 'workspace-archive-apply-invalid' });
                    return;
                }
                const result = await service.execute(body.token, body.nonce);
                if (result.archived.length > 0) invalidateRecycleCaches(result.archived);
                send(res, result.archived.length > 0 ? 200 : 409, { ok: result.archived.length > 0, ...result });
            } catch (error) { workspaceArchiveError(res, error, 'workspace-archive-apply-failed'); }
        },
    }), 'archived-chats: workspace archive apply route');
    ...
    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/retention/policy/preview`,
        handler: async (req, res) => {
            if (!guard(req, res)) return;
            try {
                const result = await retentionService.previewPolicy(await readBody(req, 64 * 1024));
                send(res, 200, { ok: true, ...result });
            } catch (error) {
                send(res, error.status ?? 500, { ok: false, error: error.code ?? 'retention-policy-preview-failed' });
            }
        },
    }), 'archived-chats: retention policy preview route');
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                const body = await readBody(req, 64 * 1024);
                const policy = await retentionService.savePolicy(body?.policy ?? body, body?.confirmation);
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const zip = await createExportZip({
                    plan,
                    inspect: (id) => (persistence.readSession ?? persistence.inspect).call(persistence, id),
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const archived = new Set(registry.archivedSessionIds.map(String));
                const trash = await trashStore.load();
                if (trash.status !== 'ready') throw Object.assign(new Error('recycle catalog is unavailable'), { status: 503, code: 'trash-store-unavailable' });
                for (const id of trash.records.keys()) known.add(String(id));
                for (const id of await pendingStore.load()) known.add(String(id));
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const outcome = await lifecycle.run(async () => {
                    ...
                    const trash = await trashStore.load();
                    if (trash.status !== 'ready') throw Object.assign(new Error('recycle catalog is unavailable'), { status: 503, code: 'trash-store-unavailable' });
                    for (const id of trash.records.keys()) currentIds.add(String(id));
                    for (const id of await pendingStore.load()) currentIds.add(String(id));
        ...
        kind: 'exact', path: `${ROUTE_PREFIX}/trash/empty`, handler: async (req, res) => {
            ...
            try {
                ...
                if (!Array.isArray(body.targets) || body.targets.length === 0) {
                    send(res, 400, { error: 'trashTargets-required' });
                    return;
                }
                if (body.targets.length > 2000) {
                    send(res, 400, { error: 'trashTargets-too-many' });
                    return;
                }
                const seen = new Set();
                const targets = [];
                for (const target of body.targets) {
                    if (typeof target?.sessionId !== 'string' || target.sessionId === '' || seen.has(target.sessionId)
                        || !['trashed', 'degraded'].includes(target?.state)
                        || typeof target?.trashedAt !== 'string' || target.trashedAt === ''
                        || !(typeof target?.snapshotId === 'string' || target?.snapshotId === null)
                        || !Number.isFinite(target?.bytes) || target.bytes < 0) {
                        send(res, 400, { error: 'trashTargets-invalid' });
                        return;
                    }
                    ...
                    targets.push({
                        ...
                        state: target.state,
                        trashedAt: target.trashedAt,
                        snapshotId: target.snapshotId,
                        bytes: target.bytes,
                    });
                }
                const result = await recycleService.empty(targets);
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const archivedSessionIds = await lifecycle.run(async () => {
                    ...
                    await requireNoPendingDeletion(pendingStore, [body.sessionId]);
                    await requireMainListReachability(persistence, registry, [body.sessionId]);
                ...
                send(res, error.status ?? 500, {
                    error: error.code ?? 'unarchive-failed',
                    message: String(error?.message ?? error),
                    ...(typeof error?.reason === 'string' ? { reason: error.reason } : {}),
                    ...(Array.isArray(error?.sessionIds) ? { sessionIds: error.sessionIds } : {}),
                });
            }
        },
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const archivedSessionIds = await lifecycle.run(async () => {
                    ...
                    await requireNoPendingDeletion(pendingStore, ids);
                    await requireMainListReachability(persistence, registry, ids);
    ...
    const deleteHandler = (batch) => async (req, res) => {
        ...
        try {
            ...
            if (body.permanent === true) {
                const result = await recycleService.deleteArchived(ids);
    ...
    return { recycleService, retentionService };
}
//#endregion
...
export function apply(ctx) {
    ...
    const registerWebSurface = () => {
        ...
        const rawPersistence = ctx.get(PERSISTENCE_KEYS[0]);
        if (webServer === undefined || registry === undefined || rawPersistence === undefined) return;
        const persistence = resolvePersistenceCompat(rawPersistence);
        registered = true;
        ...
        const { recycleService, retentionService } = registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, retentionStore, lifecycle, importTokens, restoreAdapter);
        ctx.effect(() => startRetentionScheduler({
            recover: () => recycleService.recoverStartup({ legacyPendingPath: pendingFilePath() }),
            run: (options) => retentionService.runAutomatic(options),
            report: (code, count) => ctx.logger.warn(`archived-chats: recycle cleanup: ${code}${count ? ` (${count})` : ''}`),
        }), 'archived-chats: automatic recycle cleanup');
    };
    registerWebSurface();
