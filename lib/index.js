...
async function isVisibleArchivedSession(registry, pendingStore, id) {
    const key = String(id);
    if (!registry.archivedSessionIds.map(String).includes(key)) return false;
    return !(await pendingStore.load()).has(key);
}
...
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, lifecycle, importTokens, restoreAdapter) {
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const session = state.sessions.find((row) => row.id === body.sessionId);
                if (session === undefined) {
                    send(res, 404, { error: 'session-not-archived' });
                    return;
                }
                const messages = await projectedMessages.get(body.sessionId);
                if (!(await isVisibleArchivedSession(registry, pendingStore, body.sessionId))) {
                    send(res, 404, { error: 'session-not-archived' });
                    return;
                }
    ...
    const deleteHandler = (batch) => async (req, res) => {
        ...
        try {
            ...
            projectedMessages.invalidate([...deleted, ...pending]);
