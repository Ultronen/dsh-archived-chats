 ...
 *                                                     backup ZIP.
 *   POST /plugins/dsh-archived-chats/interop/inspect → bounded external JSONL
 *                                                     preview and token.
 *   POST /plugins/dsh-archived-chats/interop/export  → guarded target JSONL.
...
import { inspectCodexJsonl, exportCodexJsonl } from './interop/codex.js';
import { inspectClaudeJsonl, exportClaudeJsonl } from './interop/claude.js';
...
const INTEROP_UPLOAD_LIMIT = 8 * 1024 * 1024;
...
/** Read bounded multipart fields for the JSONL interop endpoints. */
function readInteropUpload(req) {
    return readMultipartFields(req, INTEROP_UPLOAD_LIMIT, 'interop upload is too large');
}
...
/** Parse a bounded interop export form and preserve first-seen session order. */
function readInteropSelection(req) {
    return new Promise((resolve, reject) => {
        const chunks = [];
        let bytes = 0;
        let settled = false;
        ...
        req.on('data', (chunk) => {
            ...
            const value = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
            bytes += value.length;
            if (bytes > EXPORT_BODY_LIMIT) { fail('interop export request body is too large', 413); return; }
            chunks.push(value);
        });
        req.on('end', () => {
            if (settled) return;
            try {
                ...
                const submitted = JSON.parse(encoded);
                if (!Array.isArray(submitted) || submitted.length === 0) throw new Error('sessionIds must be a non-empty array');
                if (submitted.some((id) => typeof id !== 'string' || id === '')) throw new Error('sessionIds must contain non-empty strings');
                const ids = [...new Set(submitted)];
                if (ids.length > EXPORT_SESSION_LIMIT) throw new Error(`sessionIds cannot contain more than ${EXPORT_SESSION_LIMIT} unique ids`);
                settled = true;
                resolve({
                    ids,
                    target: target.trim().toLocaleLowerCase('en-US'),
...
function createPendingStore() {
    ...
    async function save(pending) {
        ...
        await writeFile(tempPath, `${JSON.stringify({ ids: [...pending] }, null, 2)}\n`, 'utf8');
    ...
    return {
        ...
        remove: (ids) => mutate((pending) => {
            let changed = false;
            for (const id of ids) if (pending.delete(String(id))) changed = true;
            return changed;
        }),
    };
}
 ...
 */
async function listArchived(ctx, registry, persistence, titleCache, metadataStore, pendingStore) {
    const archivedIds = registry.archivedSessionIds.map(String);
    if (archivedIds.length === 0) {
        const metadata = await metadataStore.getMany([]);
        return { metadataStatus: metadata.status, sessions: [] };
    }
    ...
    const visibleRows = rows.filter((row) => !pendingIds.has(row.id));
...
async function deleteSession(ctx, registry, persistence, id, titleCache, statsService, metadataStore, pendingStore, lifecycle, requirePending = false, lockHeld = false, livePrepared = false) {
    ...
    // The registry builds this index from persistence at startup, so it is a
    // useful fallback when a fresh persistence re-list is temporarily down.
    // Without a header/location we preserve the established archive/index
    // outcome, but cannot claim the physical log was removed.
    ...
    if (header === undefined) throw Object.assign(new Error('session location unavailable'), { code: 'session-location-unavailable' });
    ...
    const commit = async () => {
        ...
        try { await metadataStore.remove([id]); }
            ...
            ctx.logger.warn(`archived-chats: metadata cleanup for ${id} failed: ${String(error?.code ?? error)}`);
        }
...
function textFromArchiveContent(content) {
    ...
    return content.map((block) => {
        ...
        if (block !== null && typeof block === 'object') {
            ...
            return textFromArchiveContent(block.content);
        }
        return '';
    }).join('');
}

/** Project one archived append event into the generic adapter session shape. */
function archiveMessage(event) {
    if (!isAppendSurfaceEvent(event)) return null;
    let message;
    try { message = deriveEventMessage(event); }
    catch { return null; }
    if (message === null || typeof message !== 'object') return null;
    const blocks = Array.isArray(message.content) ? message.content : [message.content];
    let role = message.source?.kind === 'tool' ? 'tool' : (typeof message.role === 'string' ? message.role : 'system');
    let content = '';
    let toolCall;
    let toolCallId;
    const attachments = [];
    for (const block of blocks) {
        if (block === null || typeof block !== 'object') {
            content += typeof block === 'string' ? block : '';
            continue;
        }
        const type = typeof block.type === 'string' ? block.type.toLocaleLowerCase('en-US') : '';
        if (type === 'tool-call' || type === 'function_call') {
            toolCall = { name: typeof block.name === 'string' ? block.name : '', arguments: block.arguments ?? {} };
            if (typeof block.id === 'string' && block.id !== '') toolCallId = block.id;
        } else if (type === 'tool-result' || type === 'function_call_output') {
            role = 'tool';
            content += textFromArchiveContent(block.content ?? block.output ?? block.result);
            if (typeof block.toolCallId === 'string' && block.toolCallId !== '') toolCallId = block.toolCallId;
        } else if (type === 'image' || type === 'document' || type === 'file' || type === 'attachment') {
            const attachment = block.attachment ?? block;
            if (typeof attachment.name === 'string' && attachment.name !== '') attachments.push({
                path: attachment.name,
                ...(typeof attachment.mediaType === 'string' ? { mediaType: attachment.mediaType } : {}),
            });
        } else {
            content += typeof block.text === 'string' ? block.text : textFromArchiveContent(block.content);
        }
    }
    const value = {
    ...
    if (toolCall !== undefined) value.toolCall = toolCall;
    if (toolCallId !== undefined) value.toolCallId = toolCallId;
    return { message: value, attachments };
}

function archiveSessionForInterop(row, inspected) {
    const messages = [];
    const attachments = [];
    for (const event of Array.isArray(inspected?.events) ? inspected.events.slice(0, 20000) : []) {
        const mapped = archiveMessage(event);
        if (mapped === null) continue;
        messages.push(mapped.message);
        for (const attachment of mapped.attachments) {
            if (!attachments.some((item) => item.path === attachment.path)) attachments.push(attachment);
        }
    }
    return {
        id: String(row.id),
        title: typeof row.title === 'string' && row.title !== '' ? row.title : String(row.id),
        workspace: { id: row.workspaceId ?? null, title: row.workspaceTitle ?? null },
        messages,
        attachments,
    };
}

/** Collapse adapter diagnostics into bounded, message-free categories for UI preview. */
function summarizeInteropDiagnostics(reports, field) {
    const grouped = new Map();
    for (const report of reports) {
        for (const item of Array.isArray(report?.[field]) ? report[field].slice(0, 1000) : []) {
            const code = typeof item?.code === 'string' && item.code !== '' ? item.code.slice(0, 160) : `${field}-unknown`;
            const detail = ['role', 'eventType', 'type', 'reason']
                .map((key) => typeof item?.[key] === 'string' && item[key] !== '' ? `${key}:${item[key].slice(0, 160)}` : '')
                .filter(Boolean)
                .join('|');
            const key = `${code}\u0000${detail}`;
            const count = Number.isSafeInteger(item?.count) && item.count > 0 ? item.count : 1;
            const current = grouped.get(key) ?? { code, ...(detail === '' ? {} : { detail }), count: 0 };
            current.count += count;
            grouped.set(key, current);
        }
    }
    return [...grouped.values()].slice(0, 1000);
}

function createInteropExportPreview(target, reports) {
    const sessions = reports.flatMap((report) => (Array.isArray(report?.sessions) ? report.sessions : []))
        .slice(0, EXPORT_SESSION_LIMIT)
        .map((session) => ({
            id: typeof session?.id === 'string' ? session.id : '',
            title: typeof session?.title === 'string' ? session.title : '',
        }));
    const losses = summarizeInteropDiagnostics(reports, 'losses');
    const warnings = summarizeInteropDiagnostics(reports, 'warnings');
    return {
        source: target,
        sessions,
        losses,
        ...
        summary: {
            ...
            losses: losses.reduce((total, item) => total + item.count, 0),
            conflicts: 0,
            warnings: warnings.reduce((total, item) => total + item.count, 0),
        },
    };
}

function interopToolArguments(value) {
    if (typeof value === 'string') return value;
    try { return JSON.stringify(value ?? {}); }
    catch { return '{}'; }
}

/** Convert one bounded interop projection into a valid Harness seed log. */
function interopRestoreEvents(session, createdAt) {
    const events = [];
    let turn = 0;
    const append = (type, data, extra = {}) => {
        const seq = events.length;
        events.push({ type, seq, time: createdAt + seq, data, ...extra });
    };
    append('session/title', { title: session.title });
    for (let index = 0; index < session.messages.length; index += 1) {
        const message = session.messages[index];
        const id = typeof message.id === 'string' && message.id !== ''
            ? message.id
            : `interop-${session.id}-${index + 1}`;
        const content = textFromArchiveContent(message.content);
        if (message.role === 'user' || message.role === 'system') {
            append('user/message', {
                id,
                role: 'user',
                source: { kind: message.role === 'system' ? 'interop-system' : 'user' },
        ...
        if (message.role === 'assistant') {
            const blocks = [];
            if (content !== '' || message.toolCall === undefined) blocks.push({ type: 'text', text: content });
            if (message.toolCall !== undefined) {
                const callId = typeof message.toolCallId === 'string' && message.toolCallId !== ''
                    ? message.toolCallId
                    : `interop-call-${session.id}-${index + 1}`;
                const name = typeof message.toolCall?.name === 'string' && message.toolCall.name !== ''
                    ? message.toolCall.name
                    : 'external_tool';
                const argumentsValue = interopToolArguments(message.toolCall?.arguments);
                blocks.push({ type: 'tool-call', id: callId, name, arguments: argumentsValue });
                append('assistant/message', {
                    ...
                    message: {
                        id,
                        role: 'assistant',
                        source: { kind: 'model', provider: 'interop', model: session.source || 'external' },
                        content: blocks,
                    },
                }, { surfaceOp: 'append' });
                append('tool/call', { turn, step: 1, callId, name, arguments: argumentsValue });
            } else {
                append('assistant/message', {
                    turn,
                    step: 1,
            ...
            const callId = typeof message.toolCallId === 'string' && message.toolCallId !== ''
                ? message.toolCallId
                : `interop-call-${session.id}-${index + 1}`;
            append('tool/call', { turn, step: 1, callId, name: 'external_tool', arguments: '{}' });
            append('tool/result', {
                turn,
                step: 1,
                message: {
...
/** Create the existing restore adapter item shape without retaining source JSONL. */
function interopRestoreItem(session, createdAt) {
    const workspace = session.workspace === null ? null : {
        id: session.workspace?.id ?? null,
        title: session.workspace?.title ?? null,
    };
    const meta = { version: 0, id: session.id, createdAt };
    if (typeof workspace?.id === 'string' && isAbsolute(workspace.id)) meta.cwd = workspace.id;
    return {
        id: session.id,
        title: session.title,
        workspace,
        tags: [],
        note: '',
        hasAttachmentReferences: session.attachments.length > 0,
        record: {
            ...
            archive: {
                ...
                workspace,
                createdAt,
                origin: `interop:${session.source}`,
                metadataUpdatedAt: null,
                tags: [],
                note: '',
                storage: { status: 'unavailable', sizeBytes: null, fileCount: null },
            },
            source: { meta, events: interopRestoreEvents(session, createdAt) },
        },
    };
}
//#endregion
...
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, lifecycle, importTokens, restoreAdapter) {
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            try {
                send(res, 200, await listArchived(ctx, registry, persistence, titleCache, metadataStore, pendingStore));
            } catch (error) {
    ...
    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/interop/inspect`,
        handler: async (req, res) => {
            if (!guard(req, res)) return;
            try {
                const fields = await readInteropUpload(req);
                const sourceValue = fields.get('source');
                const source = sourceValue === undefined
                    ? ''
                    : new TextDecoder('utf-8', { fatal: true }).decode(sourceValue).trim().toLocaleLowerCase('en-US');
                if (source === '') {
                    send(res, 400, { ok: false, error: 'source-required' });
                    return;
                }
                const inspect = source === 'codex' ? inspectCodexJsonl
                    : (source === 'claude' || source === 'claude-code' ? inspectClaudeJsonl : null);
                if (inspect === null) {
                    send(res, 400, { ok: false, error: 'unsupported-source' });
                    return;
                }
                const bytes = fields.get('file');
                if (bytes === undefined || bytes.byteLength === 0) {
                    send(res, 400, { ok: false, error: 'file-required' });
                    return;
                }
                const inspected = inspect(bytes, { maxBytes: INTEROP_UPLOAD_LIMIT });
                const known = new Set();
                try { for (const header of await persistence.list()) known.add(String(header.id)); }
                catch { /* archive conflicts remain protected by the registry set */ }
                const archived = new Set(registry.archivedSessionIds.map(String));
                const conflicts = [];
                const sessions = inspected.sessions.map((session) => {
                    const conflict = known.has(String(session.id)) || archived.has(String(session.id));
                    if (conflict) conflicts.push({ id: session.id, code: 'session-conflict' });
                    return { ...session, conflict };
                });
                const report = {
                    ...inspected.report,
                    conflicts,
                    summary: { ...inspected.report.summary, conflicts: conflicts.length },
                };
                const createdAt = Date.now();
                const items = sessions.map((session) => interopRestoreItem(session, createdAt));
                const publicReport = {
                    source,
                    losses: summarizeInteropDiagnostics([report], 'losses'),
                    warnings: summarizeInteropDiagnostics([report], 'warnings'),
                    conflicts: conflicts.map((conflict) => ({ id: conflict.id, code: conflict.code })),
                ...
                const publicSessions = sessions.map((session) => ({
                    ...
                    hasAttachmentReferences: Array.isArray(session.attachments) && session.attachments.length > 0,
                    lossCount: Array.isArray(session.losses) ? session.losses.length : 0,
                }));
                const token = importTokens.create({ kind: 'interop', source, items, report: publicReport });
                send(res, 200, {
                    ok: true,
                    token: token.token,
                    nonce: token.nonce,
                ...
                send(res, error.status ?? 400, { ok: false, error: error.code ?? 'interop-inspect-failed', message: String(error?.message ?? error) });
            }
        },
    }), 'archived-chats: interop inspect route');

    ctx.effect(() => webServer.register({
        kind: 'exact',
        path: `${ROUTE_PREFIX}/interop/export`,
        handler: async (req, res) => {
            if (!guard(req, res)) return;
            try {
                const selection = await readInteropSelection(req);
                const target = selection.target === 'codex' ? 'codex'
                    : (selection.target === 'claude' || selection.target === 'claude-code' ? 'claude' : null);
                if (target === null) {
                    send(res, 400, { ok: false, error: 'unsupported-target' });
                    return;
                }
                const pending = await pendingStore.load();
                const visible = new Set(registry.archivedSessionIds.map(String).filter((id) => !pending.has(id)));
                if (selection.ids.some((id) => !visible.has(id))) {
                    send(res, 404, { ok: false, error: 'session-not-archived' });
                    return;
                }
                const state = await listArchived(ctx, registry, persistence, titleCache, metadataStore, pendingStore);
                const rows = new Map(state.sessions.map((row) => [row.id, row]));
                if (selection.ids.some((id) => !rows.has(id))) {
                    send(res, 404, { ok: false, error: 'session-not-archived' });
                    return;
                }
                const exporter = target === 'codex' ? exportCodexJsonl : exportClaudeJsonl;
                const chunks = [];
                const reports = [];
                for (const id of selection.ids) {
                    const inspected = await persistence.inspect(id);
                    const session = archiveSessionForInterop(rows.get(id), inspected);
                    const exported = exporter(session);
                    reports.push(exported?.report ?? {});
                    if (!selection.preview && exported?.bytes instanceof Uint8Array) chunks.push(exported.bytes);
                }
                if (selection.preview) {
                    send(res, 200, { ok: true, target, report: createInteropExportPreview(target, reports) });
                    return;
                }
                ...
                res.writeHead(200, {
                    ...
                    'content-disposition': contentDisposition(`dsh-archived-chats-${target}-${date}.jsonl`),
                    'cache-control': 'no-store',
                    'x-content-type-options': 'nosniff',
                });
                ...
                send(res, status, { ok: false, error: status === 413 ? 'request-too-large' : 'interop-export-failed', message: String(error?.message ?? error) });
            }
        },
    }), 'archived-chats: interop export route');
        ...
        try {
            ...
            for (const id of ids) {
                try {
                    ...
                    else if (outcome === 'deleted') deleted.push(id);
                    else failed.push({ id, code: 'delete-cancelled', message: 'session was unarchived before physical deletion' });
                } catch (error) {
