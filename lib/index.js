...
function registerRoutes(ctx, webServer, registry, persistence, titleCache, metadataStore, statsService, pendingStore, trashStore, snapshotStore, retentionStore, lifecycle, importTokens, restoreAdapter) {
    ...
    const enrichWorkspaceArchivePreview = async (preview) => {
        ...
        const headerById = new Map((Array.isArray(headers) ? headers : []).map((header) => [String(header?.id), header]));
        const sessions = await Promise.all(preview.sessions.map(async (session) => {
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            if (workspaceBulkArchiveService === null) { workspaceArchiveUnavailable(res); return; }
            try { send(res, 200, { ok: true, workspaces: await workspaceBulkArchiveService.listWorkspaces() }); }
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                send(res, 200, { ok: true, ...(await enrichWorkspaceArchivePreview(await workspaceBulkArchiveService.preview(body.workspaceId))) });
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const result = await workspaceBulkArchiveService.execute(body.token, body.nonce);
