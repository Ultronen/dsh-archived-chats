    ...
    return new Promise((resolve, reject) => {
        ...
        req.on('end', () => {
            ...
            try {
                ...
                settled = true;
                resolve({ ids, target: target.trim().toLocaleLowerCase('en-US') });
            } catch (error) { fail(String(error?.message ?? error)); }
        });
        req.on('error', (error) => {
            if (settled) return;
    ...
    ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const token = importTokens.create({ kind: 'interop', source, items, report });
                send(res, 200, {
                    ok: true,
                    token: token.token,
                    nonce: token.nonce,
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                for (const id of selection.ids) {
                    ...
                    if (exported?.bytes instanceof Uint8Array) chunks.push(exported.bytes);
                }
