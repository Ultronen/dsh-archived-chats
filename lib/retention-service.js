...
import { createAutoRetention } from './auto-retention.js';
...
export function createRetentionService({
  ...
  const automatic = createAutoRetention({ retentionStore, trashStore, recycleService, lifecycle,
    invalidate: () => insightsService.invalidate(), now, randomBytes, tokenTtlMs: ttl });
  ...
  async function applyTrash(candidate) {
    const result = await recycleService.purge([candidate.sessionId], { expected: candidate });
  ...
  async function apply(input) {
    ...
    for (const candidate of candidates) {
      try {
        if (candidate.action === 'purge-trash') await applyTrash(candidate);
        else await lifecycle.run(async () => {
  ...
  return Object.freeze({ get, ...automatic, preview, apply });
}
