...
截图 01–09 来自 `0.11.0`，空间与血缘截图来自 `0.12.0`；全部在隔离的真实 DeepSeek Harness `0.1.1-rc.2` 中文浅色 Web profile 中捕获，只使用演示会话，不包含真实用户数据。
...
![空间分析与保留策略](assets/screenshots/10-storage-retention.png)
![只读会话血缘树](assets/screenshots/11-session-lineage.png)
...
6. 打开 **空间与策略** 查看会话目录、保护快照和重复快照附件的分账；保存保留策略不会执行清理，必须再预览并确认具体候选。**会话血缘** 以只读树展示分叉与子代理关系。
...
- **空间分析**：分别统计归档/回收会话目录与插件保护快照，标出无法统计项、降级快照和重复快照附件字节；不会把这些数字描述为 Harness 全局附件可回收空间。
- **预览优先的保留策略**：可按每会话历史数、快照年龄、快照容量和回收站年龄生成候选；保存策略绝不自动执行，活动/降级快照不会被选中，回收站永久删除默认不勾选。
- **只读会话血缘**：使用 Harness 持久化 `parentSession` 展示分叉和子代理树，诊断缺失父节点、循环与委派深度不一致；本版不修改或修复血缘。
- **自动保护快照与历史**：移入回收站前保存完整会话事件和经校验的图片附件字节。重复恢复/回收会保留旧的有效保护快照，直到用户明确应用保留策略或永久删除该会话。
...
- 新增只读会话血缘树，展示分叉、子代理、缺失父节点、循环和委派深度异常。
