...
总览与外部导入预览截图来自 `0.9.0` 在本地 DeepSeek Harness `0.1.0-rc.8` Web profile 中的实际操作；其余截图保留 0.8.x 中未改变的工作流。
...
4. 从 Codex 或 Claude Code 迁移时，先选择外部来源，再点击 **从外部工具导入** 并选择 JSONL 文件；查看转换报告、信息损失、警告和 ID 冲突后，只确认需要恢复的非冲突会话。
5. 要把归档会话交接给 Codex 或 Claude Code，选择一个或多个会话（不选择时使用全部会话），点击 **导出到外部工具**，确认目标和原生继续限制后下载 JSONL。
6. 使用 **导出备份** 导出单条、当前选中项或全部归档会话。恢复备份时点击 **导入备份**，先查看预览，保留无冲突会话的勾选状态后确认恢复。
...
- **Codex / Claude Code 互操作**：只读检查外部 JSONL，在写入前展示会话数、信息损失、冲突、警告和保真度；也可把选中的 DSH 归档会话导出为面向目标工具的可读 JSONL 交接稿。
...
附件只保留安全的相对引用，**不会复制或恢复二进制文件**。外部导出的 JSONL 是用于阅读、迁移或交接的对话投影，不承诺 Codex 或 Claude Code 能原生 `resume`；导出不会修改 DSH 会话，也不会接触目标工具的源目录、凭据、MCP 密钥或本地配置。
...
- 新增面向 Codex / Claude Code 的可读迁移与交接导出，并明确标注不支持原生继续和附件二进制。
