...
以下图片使用隔离的简体中文浅色 Web 演示环境和合成会话，展示当前正式版本的真实界面，不包含真实用户数据、路径、备注或凭据。README 与插件市场使用同一套固定演示图片。
...
- 该版本将中文 README 设为仓库和 npm 包的默认入口，并另外提供英文文档。
...
卸载只会移除插件包，不会删除 `$DSH_HOME/plugin-data/archived-chats/` 中的本地数据，也不会触发回收站的永久删除。保留的数据包括 `metadata.json`、`trash.json`、`retention.json`、`snapshots/` 目录，以及尚未迁移时可能存在的旧版 `pending-deletions.json`。这样可以避免卸载时意外丢失恢复信息，之后重新安装插件仍可继续使用这些数据。准备彻底删除该目录前，请先恢复并备份需要的内容；只有确认其中的数据都不再需要时，才手动删除整个目录。
