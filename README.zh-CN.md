...
| **归档整个工作区** | 从工作区行的省略号菜单打开一次确认，归档该工作区内全部符合条件的聊天；工作区及其目录保持不变。 |
...
- **确认工作区归档：** 插件在后台准备精确集合，再显示一次包含工作区与会话数量的确认。5 分钟有效、只能使用一次的凭据不会纳入之后新建的聊天；运行中的聊天只会跳过，绝不停止或移动。
...
| 工作区操作 slot + 公开 `archiveSession` | 从现有工作区的省略号菜单执行一次确认归档。旧版 Host 会省略该菜单项；直接调用后端但缺少归档能力时返回 `workspace-archive-unsupported`，且不作任何修改。 |
