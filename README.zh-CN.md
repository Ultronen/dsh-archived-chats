...
| **归档整个工作区** | 从「归档」页预览并确认一个现有工作区中的全部符合条件聊天；工作区及其目录保持不变。 |
...
- **确认工作区归档：** 5 分钟有效、只能使用一次的确认凭据绑定预览时的精确集合。之后新建的聊天不会纳入；运行中的聊天只会跳过，绝不停止或移动。
...
| 公开 `archiveSession` | 对一个现有工作区执行预览优先的批量归档。旧版 Host 缺少此能力时返回 `workspace-archive-unsupported`，且不作任何修改。 |
